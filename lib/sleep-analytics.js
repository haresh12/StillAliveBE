...
function computeStats(logs, targetHours = 8) {
  if (!logs.length) return {
    ...
    avg_wakings: 0, avg_energy: 0,
    nights_with_stages: 0, avg_deep_min: 0, avg_rem_min: 0, avg_light_min: 0, avg_awake_min: 0,
    deep_pct: 0, rem_pct: 0,
    streak: 0, longest_streak: 0,
    debt_hours_7d: 0, net_hours_period: 0,
  ...
  const wakes_n = logs.map(l => Number(l.night_wakings || 0));
  const energy  = logs.map(l => Number(l.morning_energy || 0)).filter(Boolean);
  const dates = [...new Set(logs.map(l => l.date_str).filter(Boolean))];
  const stk   = computeStreak(logs);

  // ── Sleep stages / depth (sensor nights only — manual logs have no hypnogram) ──
  // The most-asked-for "what's actually happening": deep, REM, light, awake minutes per night.
  const stageLogs = logs.filter(l => l.sensed && l.sensed.stage_minutes);
  const sm = (k) => stageLogs.map(l => Number(l.sensed.stage_minutes[k] || 0));
  const avgDeep  = stageLogs.length ? Math.round(mean(sm('deep')))  : 0;
  const avgRem   = stageLogs.length ? Math.round(mean(sm('rem')))   : 0;
  const avgLight = stageLogs.length ? Math.round(mean(sm('light'))) : 0;
  const avgAwake = stageLogs.length ? Math.round(mean(sm('awake'))) : 0;
  const asleepMin = avgDeep + avgRem + avgLight; // exclude awake from the "% of sleep" base
  ...
  // Range-aware NET over EVERY night in the window: positive = debt owed, negative = banked surplus.
  // (logs here is already the selected period, so 1M/3M/1Y each report their own running balance.)
  const netPeriod = logs.reduce((s, l) => s + (targetHours - Number(l.total_sleep_hours || 0)), 0);
  ...
  return {
    ...
    avg_wakings:       Math.round(mean(wakes_n) * 10) / 10,
    avg_energy:        energy.length ? Math.round(mean(energy) * 10) / 10 : 0,
    nights_with_stages: stageLogs.length,
    avg_deep_min:      avgDeep,
    avg_rem_min:       avgRem,
    avg_light_min:     avgLight,
    avg_awake_min:     avgAwake,
    deep_pct:          asleepMin ? Math.round((avgDeep / asleepMin) * 100) : 0,
    rem_pct:           asleepMin ? Math.round((avgRem / asleepMin) * 100) : 0,
    streak:            stk.streak,
    grace_used:        stk.grace_used,
    longest_streak:    stk.streak,
    debt_hours_7d:     Math.round(debt * 10) / 10,
    net_hours_period:  Math.round(netPeriod * 10) / 10,
...
function buildSignal(logs) {
  ...
  return sorted.map(l => ({
    ...
    wakings:    Number(l.night_wakings || 0),
    deep:       l.sensed && l.sensed.stage_minutes ? Number(l.sensed.stage_minutes.deep || 0) : 0,
    rem:        l.sensed && l.sensed.stage_minutes ? Number(l.sensed.stage_minutes.rem || 0) : 0,
  }));
}
...
// ════════════════════════════════════════════════════════════════════
async function loadAnalysisV2(deviceId, periodDays, { openai, targetHours = 8, sleepDocRef = null, clampStartDate = null } = {}) {
  // bc-aware: a caller (sleep.bc.agent) passes sleepDocRef pointing at wellness_bc_users/{id}/agents/sleep.
  // Default (no ref) = the legacy wellness_users path — unchanged behaviour for the legacy agent.
  const ref = sleepDocRef || (deviceId ? sleepDoc(deviceId) : null);
  if (!ref) return null;

  const logsCol = ref.collection('sleep_logs');
  const [sleepSnap, allSnap] = await Promise.all([
    ref.get(),
    logsCol.orderBy('logged_at', 'desc').limit(180).get(),
  ]);

  let allLogs = allSnap.docs.map(d => ({
    id: d.id,
    ...d.data(),
    logged_at: d.data().logged_at?.toDate?.() || new Date(),
  }));
  // 🚨 Registration-anchor law: never count nights logged before the user registered.
  if (clampStartDate) allLogs = allLogs.filter(l => typeof l.date_str === 'string' && l.date_str >= clampStartDate);
...
async function loadTrackContext(deviceId, { targetHours = 8, sleepDocRef = null } = {}) {
  const ref = sleepDocRef || (deviceId ? sleepDoc(deviceId) : null);
  if (!ref) return null;
  const logsCol = ref.collection('sleep_logs');
