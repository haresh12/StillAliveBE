...
async function getOrGenAiHero(deviceId, openai, statsPayload, language = 'en') {
  ...
  // HK-enrichment — fetched once, used by both the hash and the user message
  // so cached results invalidate when HK signals change.
  const { withHKEnrichment, hkCacheToken, HK_PROMPT_RULE } = require('./healthkit/analytics-helper');
  const hkToken = await hkCacheToken({ deviceId, coach: 'sleep', admin });
  const hashWithHK = contentHash({ ...statsPayload, _hk: hkToken });

  try {
    const userMsg = await withHKEnrichment({ deviceId, coach: 'sleep', payload: statsPayload, admin });
    const completion = await openai.chat.completions.create({
      model: AI.REASONING_FAST,
      response_format: { type: 'json_object' },
      max_completion_tokens: 220,
      messages: [
        { role: 'system', content: appendLanguageInstruction(`${SLEEP_HERO_SYSTEM}\n\n${HK_PROMPT_RULE}`, language) },
        { role: 'user',   content: userMsg },
      ],
    });
    const parsed = JSON.parse(completion.choices?.[0]?.message?.content || '{}');
    const insight = parsed?.headline || null;
    if (insight) cacheRef.set({ hash: hashWithHK, insight, generated_at: admin.firestore.FieldValue.serverTimestamp() }).catch(() => {});
    return insight;
  } catch (err) {
...
async function getOrGenAiReads(deviceId, openai, payload, language = 'en') {
  const { withHKEnrichment, hkCacheToken, HK_PROMPT_RULE } = require('./healthkit/analytics-helper');
  const hkToken = await hkCacheToken({ deviceId, coach: 'sleep', admin });
  const hash = contentHash({
    ...
    drift: payload.weekend_drift, _hk: hkToken,
    lang: language,
  });
  ...
  try {
    const userMsg = await withHKEnrichment({ deviceId, coach: 'sleep', payload, admin });
    const completion = await openai.chat.completions.create({
      model: AI.REASONING_FAST,
      response_format: { type: 'json_object' },
      max_completion_tokens: 700,
      messages: [
        { role: 'system', content: appendLanguageInstruction(`${SLEEP_AI_READS_SYSTEM}\n\n${HK_PROMPT_RULE}`, language) },
        { role: 'user',   content: userMsg },
      ],
    });
    const parsed = JSON.parse(completion.choices?.[0]?.message?.content || '{}');
...
// ════════════════════════════════════════════════════════════════════
async function loadAnalysisV2(deviceId, periodDays, { openai, targetHours = 8, language = 'en' } = {}) {
  ...
  // daily_logs is the LIFETIME map (always all logs since anchor) — both
  // the 28-day Calendar and the 365-day Journey heatmap consume it. The
  // 28-day Calendar internally filters to its window, so giving it the
  // full map is safe. Previously this was period-filtered which left the
  // Journey heatmap blank whenever the user selected 1W.
  const daily  = buildDailyLogs(allLogs);
  ...
  if (openai && periodStats.total_logs >= 1) {
    // ── 2026-05-25 deadline fix ──
    // Both AI calls already cache by content-hash. On cold-cache the LLM
    // round-trip can run 8-25s; the FE safeFetch was timing out at 15s
    // and surfacing a red "[SleepV4] Request timed out" error to users.
    //
    // Cap each AI call at 8s. If it doesn't return in time we:
    //   • Resolve with null/[] so /analysis returns a fast response
    //   • LET the underlying promise keep running — its `then` still
    //     writes to the cache doc, so the NEXT /analysis call serves
    //     the AI insight instantly from cache.
    // Net: worst-case cold-cache /analysis ≈ 9s, well under 15s.
    const withDeadline = (p, ms, fallback) => Promise.race([
      p.catch(() => fallback),
      new Promise((r) => setTimeout(() => r(fallback), ms)),
    ]);
    [heroInsight, aiReads] = await Promise.all([
      withDeadline(getOrGenAiHero(deviceId, openai, heroPayload, language), 8000, null),
      withDeadline(getOrGenAiReads(deviceId, openai, readsPayload, language), 8000, []),
    ]);
  }
