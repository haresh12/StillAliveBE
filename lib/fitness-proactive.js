...
async function craftLine(deviceId, situation, fallback) {
  try {
    const mem = await retrieveMemories(deviceId, situation, 4);
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a sharp, warm personal fitness coach texting your client UNPROMPTED. Write ONE short line (<=22 words). Specific and human — weave in what you know about them when it fits. NEVER generic, NEVER corporate or 'AI slop', NEVER shame a missed day. At most ONE emoji. Output ONLY the line — no quotes, no preamble.",
        },
...
async function runProactiveCheck(deviceId, opts = {}) {
  try {
    ...
    if (!pattern && [7, 14, 30, 60, 100].includes(streakDays)) {
      pattern = {
    ...
    const line = await craftLine(deviceId, pattern.situation, pattern.fallback);
...
async function craftImpactLine(deviceId, situation, fallback, recentOpeners = []) {
  try {
    const mem = await retrieveMemories(deviceId, situation, 3);
    const memBlock = mem.length ? `What you know about them:\n- ${mem.join("\n- ")}\n\n` : "";
