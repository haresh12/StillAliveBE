...
// ─── Duration policy (days) ─────────────────────────────────────────────
// v4 (2026-05-28): plans are now FREE-FORM in [MIN, MAX]. The user can
// ask for any integer day count — "5-day reset", "10 days", "21 days",
// "60 days" — and we generate exactly that many days. The only restriction
// is the per-tier ceiling enforced by freeTierClamp.js: free → 14, premium
// → 60. DURATIONS is kept ONLY as a hint list for UI presets and is no
// longer used as a wire-format validator.
const MIN_DURATION_DAYS = 3;
const MAX_DURATION_DAYS = 90;       // hard ceiling regardless of tier (LLM cost guard)
const FREE_MAX_DURATION_DAYS    = 14;
const PREMIUM_MAX_DURATION_DAYS = 60;
const DURATIONS = Object.freeze([7, 14, 30, 60]);  // UI preset chips only
...
module.exports = Object.freeze({
  ...
  MIN_DURATION_DAYS,
  MAX_DURATION_DAYS,
  FREE_MAX_DURATION_DAYS,
  PREMIUM_MAX_DURATION_DAYS,
  isValidDuration,
  ITEM_KINDS,
  QUESTION_KINDS,
  LIMITS,
