...
// Weights self-normalize per day over the agents that HAVE data (see weightSum below), so breath's
// small absolute weight just gives it a modest relative pull without rebalancing everyone's score.
const WEIGHTS = { fitness: 0.25, sleep: 0.25, mind: 0.15, nutrition: 0.15, water: 0.10, fasting: 0.10, breath: 0.05 };
...
async function fetchAllAgentData(deviceId, days = 90) {
  ...
  const [mind, sleep, water, fasting, nutrition, fitness, breath] = await Promise.all([
    ...
    agentDoc(deviceId, 'fitness').collection('fitness_workouts')
      .where('logged_at', '>=', cutoffTs).orderBy('logged_at', 'desc').limit(200).get()
      .then(s => s.docs.map(d => d.data())).catch(() => []),
    agentDoc(deviceId, 'breath').collection('breath_sessions')
      .where('logged_at', '>=', cutoffTs).orderBy('logged_at', 'desc').limit(500).get()
      .then(s => s.docs.map(d => d.data())).catch(() => []),
  ]);

  return { mind, sleep, water, fasting, nutrition, fitness, breath };
}
...
function dailyBreath(sessions) {
  const byMin = {};
  sessions.forEach(s => {
    const d = s.date_str || dateStr(millis(s.logged_at));
    byMin[d] = (byMin[d] || 0) + (s.seconds || 0) / 60;
  });
  const out = {};
  // Score the day against the 5-min evidence-based dose (Balban 2023); any real session floors at ~20.
  Object.entries(byMin).forEach(([d, min]) => { out[d] = clamp(round((min / 5) * 100) || 20, 20, 100); });
  return out;
}
...
function buildDailyMatrix(data) {
  return {
    ...
    breath:    dailyBreath(data.breath),
  };
}
