...
async function generateAhaText(correlation, language) {
...
Output ONLY the sentence. No quotes, no preamble.`;
  ...
  const langDirective = appendLanguageInstruction('', lang);
  try {
    ...
    log.warn('[cross-agent] AHA fail:', e.message);
    return `Your ${a} and ${b} are linked — focus on both together.`;
  }
}

async function getOrGenerateAha(deviceId, top, language) {
  if (!top) return null;
  const hash = correlationHash(top);
  const snap = await ahaDoc(deviceId).get();
  const cached = snap.exists ? snap.data() : {};
  // Cache key is hash + language so each locale gets its own cached text;
  // a language change won't return stale English to a German user.
  const lang = normalizeLanguage(language);
  if (cached.hash === hash && cached.language === lang && cached.text) {
  ...
  const text = await generateAhaText(top, lang);
  await bumpCost(deviceId, 'aha');
  await ahaDoc(deviceId).set({
    hash, text, language: lang, correlation: top, generated_at: admin.firestore.FieldValue.serverTimestamp(),
  });
...
async function generateMetaCoach({ score, subscores, correlations, trend7d, trend30d, language }) {
...
NO jargon. NO bullet points. Just 3 sentences. Address the user directly ("you").`;
  ...
  const langDirective = appendLanguageInstruction('', normalizeLanguage(language));
  try {
    const resp = await openai.chat.completions.create({
    ...
    log.warn('[cross-agent] meta fail:', e.message);
    return null;
  }
}

async function getOrGenerateMeta(deviceId, payload, language) {
  const week = isoWeek();
  const lang = normalizeLanguage(language);
  const snap = await metaDoc(deviceId).get();
  const cached = snap.exists ? snap.data() : {};
  // Cache key = week + language so a language switch re-generates.
  if (cached.week === week && cached.language === lang && cached.text) {
  ...
  const text = await generateMetaCoach({ ...payload, language: lang });
  if (!text) return null;
  await bumpCost(deviceId, 'meta');
  await metaDoc(deviceId).set({
    week, text, language: lang, generated_at: admin.firestore.FieldValue.serverTimestamp(),
  });
...
async function buildHomePayload(deviceId, language) {
  ...
  const aha = (maturity.stage >= 4 && correlations[0])
    ? await getOrGenerateAha(deviceId, correlations[0], language)
    : null;
...
async function buildInsightsPayload(deviceId, language) {
  ...
  if (maturity.stage >= 3) {
    ...
    meta = await getOrGenerateMeta(deviceId, { score, subscores, correlations, trend7d: t7, trend30d: t30 }, language);
  }
