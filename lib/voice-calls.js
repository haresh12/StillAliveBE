...
// Single source of truth for the account's subscription flags — reads the account-level
// subscription (synced to wellness_users; billing isn't agent-sandboxed). Three distinct ideas:
//   • hasAccess — an ACTIVE entitlement (trial OR paid). Can the user use calls AT ALL.
//   • isTrial   — currently inside the free-trial period (not yet paying).
//   • isPaid    — a CONVERTED, paying subscriber (NOT trial). ONLY this tier earns the full
//                 50-min premium budget; trial users get the same 5-min taste as free.
async function subFlags(deviceId) {
  try {
    ...
    const isTrial = !!sub.isTrial || String(sub.periodType || '').toLowerCase() === 'trial';
    const hasAccess = !!(sub.isPremium || sub.isTrial);
    return { hasAccess, isTrial, isPaid: hasAccess && !isTrial };
  } catch { return { hasAccess: false, isTrial: false, isPaid: false }; }
}

// ACCESS-level premium (trial COUNTS — trial users may use calls). The minute BUDGET is a separate
// question answered by subFlags().isPaid, so trial users keep access but only the free-tier budget.
async function isPremium(deviceId) {
  return (await subFlags(deviceId)).hasAccess;
}

async function dailyStatus(deviceId) {
  const [anchor, flags] = await Promise.all([
  ...
  // ONLY a converted (paying, non-trial) subscriber earns the 50-min budget. Trial users get the
  // same 5-min taste as free — cost protection + a natural upgrade moment when they hit the wall.
  const monthlyLimit = flags.isPaid ? PREMIUM_MONTHLY_SEC : FREE_MONTHLY_SEC;
  ...
  return {
    ...
    is_premium: flags.hasAccess, // ACCESS-level (trial counts) — unchanged for back-compat
    is_trial: flags.isTrial,     // in the free-trial period → gets the 5-min taste, upsell on empty
    is_paid: flags.isPaid,       // converted subscriber → the full 50-min budget
    last_call_at: lastMs || null,
    today,
  };
}

module.exports = { summarizeTranscript, saveCall, listCalls, getCall, dailyStatus, isPremium, subFlags, getLearnings, getRecentCallContext };
