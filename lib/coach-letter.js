...
If the user has an "healthkit" object in their context, those values are auto-imported from Apple Health and are FACTUAL. You may cite them verbatim (e.g. "your 4.6h sleep last night", "your HRV ran 18% below baseline this week"). Never invent HK numbers — only cite what's present.
...
async function getOrGenerateLetter(deviceId, ctx, harvest, scoreImpact, language) {
  const week = isoWeek();
  ...
  // HK rollup across all 6 coaches — non-fatal if HK isn't granted.
  const hkRollup = {};
  try {
    ...
    coaches.forEach((c, i) => {
      if (blocks[i] && blocks[i].trim()) hkRollup[c] = blocks[i].replace(/^\[HK\]\s*/, '');
    });
  ...
  const promptInput = {
    ...
    // Apple Health auto-imported signals — coach can cite these directly.
    // Only the keys present have data, so the letter never invents numbers.
    healthkit: Object.keys(hkRollup).length ? hkRollup : null,
  };
  ...
  try {
    ...
    log.warn('[coach-letter]', e.message);
    return cached?.payload || null;
  }
