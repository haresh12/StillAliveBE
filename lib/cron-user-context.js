...
async function getUserNotifContext(db, deviceId) {
  ...
  try {
    ...
    let language = normalizeLanguage(profile.language);
    let utcOffsetMinutes = Number.isFinite(profile.utc_offset_minutes)
      ? profile.utc_offset_minutes
      : null;
    // Notif enabled default: true (opt-out, not opt-in — matches FE policy default)
    let notifEnabled = profile.notif_enabled !== false;
    // Big-change users may store quiet-hours / DND on wellness_bc_users (aliveChecks is sparse for them).
    // Captured in the bc block below, preferred over the profile defaults so a bc user's chosen quiet hours
    // are actually honored by the crons.
    let bcHardDND = null, bcDndStart = null, bcDndEnd = null;

    // ── Big-change users store tz / language / notif prefs in wellness_bc_users,
    // NOT aliveChecks (bc onboarding never writes aliveChecks). Without this every
    // bc user resolves to server UTC → proactive crons fire at the wrong local hour
    // (3am pings, "good morning" at night, "the day just started" at bedtime).
    // Prefer the travel-fresh current_tz_offset, then the frozen registration_tz_offset
    // — the same field resolveAnchor() already trusts for scoring.
    try {
      const { DATA_NAMESPACE, ns } = require('./collections');
      if (DATA_NAMESPACE) {
        const bcSnap = await db.collection(ns('users')).doc(deviceId).get();
        const bd = (bcSnap && bcSnap.exists && bcSnap.data()) || null;
        if (bd) {
          if (utcOffsetMinutes === null) {
            if (Number.isFinite(bd.current_tz_offset)) utcOffsetMinutes = bd.current_tz_offset;
            else if (Number.isFinite(bd.registration_tz_offset)) utcOffsetMinutes = bd.registration_tz_offset;
          }
          if (!profile.language && bd.language) language = normalizeLanguage(bd.language);
          if (typeof bd.notifications_enabled === 'boolean') notifEnabled = bd.notifications_enabled;
          // Quiet hours / DND (support both dnd_* and quiet_* field names).
          if (typeof bd.do_not_disturb === 'boolean') bcHardDND = bd.do_not_disturb;
          if (bd.dnd_start || bd.quiet_start) bcDndStart = bd.dnd_start || bd.quiet_start;
          if (bd.dnd_end || bd.quiet_end) bcDndEnd = bd.dnd_end || bd.quiet_end;
        }
      }
    } catch (_) { /* never break a cron loop over tz resolution — fall back to UTC */ }

    const hardDND = bcHardDND != null ? bcHardDND === true : profile.do_not_disturb === true;

    const dndStart = parseHHMM(bcDndStart || profile.dnd_start, parseHHMM('22:00'));
    const dndEnd   = parseHHMM(bcDndEnd   || profile.dnd_end,   parseHHMM('07:00'));
...
function _fallbackContext(deviceId = null) {
  ...
  return {
    ...
    localDateStr: serverNow.toISOString().slice(0, 10),
    notifEnabled: true,
    hardDND: false,
    inDND: false,
