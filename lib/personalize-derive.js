...
// Caffeine half-life ≈ 5h. Cutoff at bedtime - 8h keeps sleep architecture
// intact for ~95% of adults (Drake et al., J Clin Sleep Med 2013). This is
// the deterministic AHA on the Rhythm screen — no LLM, no I/O, no excuse.
function deriveCaffeineCutoff(bed_min) {
  const cutoff = (bed_min - 8 * 60 + 1440) % 1440;
  return minToHHMM(cutoff);
}

// Best workout window heuristic: late-morning to early-afternoon for early
// chronotypes (cortisol-low + warm body temp), 3-5h before bed for evening
// chronotypes (best peak performance). Used as gym_time hint + AHA.
function deriveWorkoutWindow(wake_min, bed_min) {
  const chrono = deriveChronotype(wake_min);
  if (chrono === 'early') {
    return `${minToHHMM(wake_min + 4 * 60)}–${minToHHMM(wake_min + 7 * 60)}`;
  }
  if (chrono === 'evening') {
    return `${minToHHMM(bed_min - 6 * 60)}–${minToHHMM(bed_min - 3 * 60)}`;
  }
  return `${minToHHMM(wake_min + 3 * 60)}–${minToHHMM(wake_min + 6 * 60)}`;
}
...
// ─── DEFERRED FIELDS — silently filled at first-log via JIT save ──
// When the user hasn't answered these in the coach-themed flow, derive
// safe defaults so engines + scoring never read undefined. The corresponding
// FE confirm sheets call POST /api/personalize/jit-save on first log to
// upgrade these to user-answered values. Until then, scoring MUST NOT
// penalize the gap (Wellness Score V2 already defaults silently).
const DEFAULTS = {
  sleep:     { target_hours: 7.5, disruptors: [] },
  mind:      { triggers: [] },
  nutrition: { goal: 'maintain', dietary_style: ['anything'], allergies: [] },
  fitness:   { training_level: 'beginner', goal: 'general', training_days: ['mon','wed','fri'], equipment: 'any' },
  fasting:   { protocol: '14_10' },
  shared:    { wake_time_min: 420, bed_time_min: 1380, weight_kg: 70, height_cm: 170, activity_level: 'moderate', pregnancy: false },
};

function withDefaults(payload) {
  const p = { ...payload };
  p.shared    = { ...DEFAULTS.shared,    ...(payload.shared    || {}) };
  p.sleep     = { ...DEFAULTS.sleep,     ...(payload.sleep     || {}) };
  p.mind      = { ...DEFAULTS.mind,      ...(payload.mind      || {}) };
  p.nutrition = { ...DEFAULTS.nutrition, ...(payload.nutrition || {}) };
  p.fitness   = { ...DEFAULTS.fitness,   ...(payload.fitness   || {}) };
  p.fasting   = { ...DEFAULTS.fasting,   ...(payload.fasting   || {}) };
  return p;
}
