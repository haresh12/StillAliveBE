...
const { withHKEnrichmentCrossCoach, HK_PROMPT_RULE } = require('./healthkit/analytics-helper');
...
${HK_PROMPT_RULE}
If healthkit data appears, weave its numbers in naturally as part of the narrative — never name the source ("Apple Health", "your watch", "wearable"). Treat it as the user's own data.
 ...
 */
async function getOrGenerateWeeklyReport(deviceId, ctx, harvest, findings, scoreImpact, score, language) {
  // ── Eligibility ──────────────────────────────────────────────────
  ...
  // HK silent enrichment — pulls 7-day HK rollups per active coach (sleep
  // hours, HRV trend, steps, etc.). If user has no wearables, this is a
  // no-op and the payload + cache key stay identical to pre-HK behavior.
  const activeCoaches = (harvest?.contributors || []).filter((c) => c.setup).map((c) => c.agent);
  const enrichedInput = await withHKEnrichmentCrossCoach({
    deviceId,
    payload: promptInput,
  ...
  try {
    ...
    log.warn('[reports-engine][weekly]', e.message);
    return null;
  }
 ...
 * @returns {object}  report payload, or { eligible: false, reason, days_remaining }
 */
async function getOrGenerateMonthlyReport(deviceId, ctx, harvest, findings, scoreImpact, score, trend30d, language) {
  // ── Eligibility ──────────────────────────────────────────────────
  const maturityDays = score?.maturity?.days ?? ctx.days_with_any_log ?? 0;
  const setupCount   = harvest?.counts?.setup_count || 0;
  ...
  // HK silent enrichment for monthly — pulls 30d window per active coach.
  const activeCoaches = (harvest?.contributors || []).filter((c) => c.setup).map((c) => c.agent);
  const enrichedInput = await withHKEnrichmentCrossCoach({
    deviceId,
    payload: promptInput,
  ...
  try {
    ...
    log.warn('[reports-engine][monthly]', e.message);
    return null;
  }
