...
// Tier slant — a single STABLE line per tier (byte-identical → still prompt-cacheable per coach+lang+tier).
// premium/trial users are ENTITLED, so never upsell them; free users can get a gentle, non-pushy nudge.
function tierLine(tier) {
  if (tier === 'premium' || tier === 'trial') {
    return `• This person is already a member — celebrate and support them. NEVER upsell, and never mention upgrading, plans, or premium.`;
  }
  return `• This person is on the free tier — be genuinely helpful FIRST; any nudge toward more must be gentle and never pushy.`;
}

// STABLE per-coach system prefix (prompt-cached). Never interpolate per-user data here — keep it byte-
// identical across users with the same coach (+lang+tier) so the cache hits.
function systemPrompt(coach, lang, tier) {
  return [
 ...
 */
async function composeCopy(deviceId, moments, lang, tier) {
  if (!openai || !Array.isArray(moments) || !moments.length) return null;

  // Normalize tier to the two slants that matter for copy: entitled (no upsell) vs free.
  const tierNorm = (tier === 'premium' || tier === 'trial') ? 'member' : 'free';
  ...
  const key = `${coach.name}|${lang || 'en'}|${tierNorm}|${safeJson(moments.map(m => [m.kind, m.agent || '', m.vars || {}]))}`;
  ...
  try {
    const r = await openai.chat.completions.create({
      model: MODEL,
      messages: [
        { role: 'system', content: systemPrompt(coach, lang, tierNorm === 'member' ? 'premium' : 'free') },
