...
// max_completion_tokens only — never max_tokens, never temperature.
async function generateAiReads(sessions, setup, rangeMeta, fastingScore, openai, deviceId, language = 'en') {
  const completed = sessions.filter(s => s.completed && s.actual_hours > 0);
  ...
  const recentLines = completed
    .slice(0, 10)
    .map(s => {
      const d = s.date || (s.started_at ? _localDateStr(new Date(getMs(s.started_at))) : '?');
  ...
  try {
    ...
    const enriched = await withHKEnrichment({ deviceId, coach: 'fasting', payload: { stats_context: userCtx } });
    userMsg = `${userCtx}\n\nALSO_AVAILABLE_DATA:\n${enriched}`;
    hkRule = `\n\n${HK_PROMPT_RULE}`;
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: appendLanguageInstruction(`${FASTING_AI_READS_SYSTEM_PROMPT}${hkRule}`, language) },
    ...
    log.error('[fasting-analytics] generateAiReads:', e.message);
