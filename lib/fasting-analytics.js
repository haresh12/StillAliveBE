...
function computeDayOfWeek(sessions) {
  ...
  // Full per-weekday array (every day with ≥1 fast) → drives the "what days do you fast" bar graph.
  const DOW_SHORT = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const by_dow = Object.entries(byDow)
    .filter(([, v]) => v.total > 0)
    .map(([dow, v]) => ({
      dow: DOW_SHORT[Number(dow)],
      count: v.total,
  ...
  if (dowScores.length < 2) return { best_day: null, worst_day: null, by_dow };
