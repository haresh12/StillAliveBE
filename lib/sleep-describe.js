...
- OVERNIGHT RANGE with no am/pm: "11 to 7", "slept 11 to 6:30" is a night's sleep — the FIRST time is
  PM (bedtime), the SECOND is AM (wake). "11 to 7" → bedtime "23:00", wake "07:00". Never read both as AM.
- INFER THE MISSING END from a duration: if the user gives total sleep/time-in-bed PLUS only ONE clock
  time, compute the other end and report it with confidence 0.6 (it's derived, not stated). Wake = bedtime
  + duration; bedtime = wake − duration. E.g. "woke at 7, slept about 8 hours" → bedtime "23:00" (0.6),
  wake "07:00" (0.9). Do NOT invent both from a duration alone (no clock time → leave both null).
...
async function parseSleepText(openai, transcript) {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: SLEEP_DESCRIBE_SYSTEM },
