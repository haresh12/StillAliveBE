...
// namespace-aware: resolves wellness_bc_users in bc mode, wellness_users otherwise.
// Cross-agent snapshots must read the SAME namespace the user's data is written to.
const userDoc   = (id) => require('./collections').userDoc(id);
...
const AGENTS = ['fitness', 'sleep', 'mind', 'nutrition', 'water', 'fasting', 'breath'];
...
const AGENT_LOGS = {
  ...
  breath:    { col: 'breath_sessions',   ts: 'logged_at' },
};
...
function compactLog(agent, log) {
  ...
  switch (agent) {
    ...
    case 'breath':
      return { ...base, moment: log.moment, protocol: log.protocol, minutes: log.seconds ? Math.round(log.seconds / 60 * 10) / 10 : 0, feel_delta: (log.feel_before != null && log.feel_after != null) ? (log.feel_after - log.feel_before) : null };
    default: return base;
  }
}
...
async function buildContext(deviceId, opts = {}) {
  ...
  // ── Today-first status ────────────────────────────────────────────
  // What's still PENDING *today* per agent, so the coach can focus on
  // finishing today before ever suggesting "tomorrow". An agent's day is
  // "done" when it's been logged today AND has no open (active) actions.
  const today = dateStr(Date.now());
  const today_status = {};
  for (const a of AGENTS) {
    const logged_today = (recent_logs[a] || []).some(l => l.date === today);
    const pending_actions = (recent_actions[a] || [])
    ...
    today_status[a] = {
      logged_today,
      pending_actions,
      done: logged_today && pending_actions.length === 0,
    };
  }
  // Only agents the user has actually set up count toward "everything done".
  const active_agents = AGENTS.filter(a => setup_state[a] === 'setup');
  const today_all_done = active_agents.length > 0 && active_agents.every(a => today_status[a].done);
