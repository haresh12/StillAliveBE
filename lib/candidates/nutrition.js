...
async function computeNutritionCandidates(logs, setup) {
  ...
  // ════════════════════════════════════════════════════════════
  // V4 archetypes (2026-05-26) — research-anchored, parity with
  // Fitness Actions V4 deload/rebalance/effort_up/plateau_break.
  // Each derived from already-computed `days` aggregates. AI writes
  // copy on top; numbers + citations are baked in here.
  // ════════════════════════════════════════════════════════════

  // ── REFEED — extended deficit pattern ──
  // Trigger: 7+ consecutive days with avg kcal ≤ 75% of target.
  // Counterpart to Fitness "deload". Source: Trexler 2014
  // (J Int Soc Sports Nutr 11:7) — metabolic adaptation to chronic
  // deficit; periodic maintenance days restore leptin + thyroid output.
  const last7Days = days.slice(0, 7);
  const calTarget7 = Number(setup?.calorie_target || setup?.targets?.calories || 2000);
  if (last7Days.length >= 7) {
    const avgKcal7 = last7Days.reduce((s, d) => s + d.calories, 0) / 7;
    if (avgKcal7 > 0 && avgKcal7 <= calTarget7 * 0.75) {
      candidates.push({
        archetype: "refeed",
        score: 78, category: "nutrition",
        proof: { metric: "avg_kcal_7d", value: Math.round(avgKcal7), threshold: Math.round(calTarget7 * 0.85), citation: "Trexler 2014 (JISSN)" },
        proof_text: `Avg ${Math.round(avgKcal7)} kcal/day — ${Math.round((1 - avgKcal7 / calTarget7) * 100)}% below your target for 7 days. One maintenance day restores leptin + thyroid output.`,
        surprise_hook: `7 days under-eating — schedule one maintenance day this week.`,
        target: { kind: "refeed_day", calories_min: Math.round(calTarget7 * 0.95) }, success_type: "log_meal", when_to_do: "this_week", impact: 2,
      });
    }
  }

  // ── REBALANCE_MACROS — protein < 15% or fat > 40% of kcal ──
  // Counterpart to Fitness "rebalance". Source: IOM 2005 AMDR bands
  // (protein 10-35%, fat 20-35%, carb 45-65%). Out-of-band ratios
  // correlate with poor satiety + worse adherence (Drewnowski 2018).
  if (last7Days.length >= 5) {
    const totalP = last7Days.reduce((s, d) => s + d.protein, 0);
    const totalC = last7Days.reduce((s, d) => s + d.calories, 0); // total kcal already
    const totalProtKcal = totalP * 4;
    if (totalC > 0) {
      const protPct = Math.round((totalProtKcal / totalC) * 100);
      if (protPct < 15 && totalP > 0) {
        candidates.push({
          archetype: "rebalance_macros",
          score: 72, category: "nutrition",
          proof: { metric: "protein_pct_kcal_7d", value: protPct, threshold: 15, citation: "IOM 2005 AMDR" },
          proof_text: `Protein is only ${protPct}% of your weekly calories. AMDR floor is 10-35% — under 15% predicts poor satiety + lean-mass loss in deficit.`,
          surprise_hook: `Protein at ${protPct}% of kcal — below the AMDR floor.`,
          target: { protein_g: proteinTargetG }, success_type: "hit_protein", when_to_do: "this_week", impact: 2,
        });
      }
    }
  }

  // ── FIBER_PUSH — chronic low fiber ──
  // Counterpart to Fitness "effort_up". Source: POUNDS Lost (PMC6768815)
  // — fiber independently predicts weight-loss adherence regardless of
  // macro split. Drewnowski 2018 — dietary diversity + fiber drive
  // micronutrient adequacy.
  if (last7Days.length >= 5) {
    const avgFiber7 = last7Days.reduce((s, d) => s + d.fiber, 0) / last7Days.length;
    if (avgFiber7 > 0 && avgFiber7 < 18) {
      candidates.push({
        archetype: "fiber_push",
        score: 68, category: "nutrition",
        proof: { metric: "avg_fiber_7d", value: Math.round(avgFiber7), threshold: 25, citation: "POUNDS Lost (PMC6768815)" },
        proof_text: `Avg fiber ${Math.round(avgFiber7)}g/day vs ${fiberTargetG}g target. Fiber independently predicts adherence — strongest single lever after protein.`,
        surprise_hook: `Fiber averaging ${Math.round(avgFiber7)}g — push toward 25g/day.`,
        target: { fiber_g: fiberTargetG }, success_type: "hit_protein", when_to_do: "this_week", impact: 2,
      });
    }
  }

  // ── PROTEIN_PLATEAU_BREAK — protein stuck at 50-70% adherence ──
  // Counterpart to Fitness "plateau_break". When the user is logging
  // consistently but missing protein 30-50% of the time over 21+ days,
  // the standard "hit your target" nudge has been ignored — surface a
  // tactical change (front-load breakfast protein).
  // Source: ISSN 2017 (Jäger et al.) — front-loading distribution wins
  // over evening-heavy patterns for muscle protein synthesis.
  const last21 = days.slice(0, 21);
  if (last21.length >= 14) {
    const proteinAdh21 = last21.filter(d => d.protein >= proteinTargetG * 0.9).length / last21.length;
    if (proteinAdh21 >= 0.30 && proteinAdh21 <= 0.65) {
      // Inspect WHEN protein lands during a typical day for front-load hint
      const breakfastProtein = last21
        .flatMap(d => d.meals.filter(m => m.type === 'breakfast' || (m.hour != null && m.hour < 11)))
        .reduce((s, m) => s + (Number(m.protein_g || 0) || 0), 0) / Math.max(last21.length, 1);
      candidates.push({
        archetype: "protein_plateau_break",
        score: 70, category: "nutrition",
        proof: { metric: "protein_adherence_21d", value: Math.round(proteinAdh21 * 100), threshold: 80, citation: "ISSN 2017 (Jäger et al.)" },
        proof_text: `Hitting protein ${Math.round(proteinAdh21 * 100)}% of days for 3 weeks — stuck pattern. Front-loading breakfast protein (target 35-40g AM) flips evening-heavy users to hit targets.`,
        surprise_hook: `21 days at ${Math.round(proteinAdh21 * 100)}% protein adherence — try breakfast-first.`,
        target: { protein_g: proteinTargetG, breakfast_protein_g: 35 }, success_type: "hit_protein", when_to_do: "today", impact: 3,
      });
    }
  }
