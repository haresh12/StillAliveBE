...
function computeStats(checkins) {
  if (!checkins.length) return {
    ...
    streak: 0, longest_streak: 0, pct_calm_days: null,
  };
  ...
  // pct_calm_days: share of LOGGED days whose day-avg anxiety was calm (<= 2 on the 1-5 scale,
  // where 1=calm). Per-day mean so multiple check-ins in a day count once.
  const anxByDate = {};
  for (const c of checkins) {
    if (!c.date_str) continue;
    (anxByDate[c.date_str] = anxByDate[c.date_str] || []).push(Number(c.anxiety_level || c.anxiety || 1));
  }
  const calmDayCount = Object.values(anxByDate).filter(arr => mean(arr) <= 2).length;
  return {
    ...
    pct_calm_days:   Math.round((calmDayCount / Math.max(1, dates.length)) * 100),
  };
}
...
const MIND_HERO_SYSTEM = `You are a warm, plain-spoken mood coach. Write the ONE-sentence opening insight on a personal mental-wellness app's Insights tab. ONE sentence that makes the user feel "this is exactly what's going on with me."
...
const MIND_AI_READS_SYSTEM = `You are a warm, plain-spoken mood coach writing the "What I'm seeing" section. The user has logged mood, anxiety, emotions, triggers, and notes. Produce exactly THREE distinct reads that feel like a friend who knows their patterns.
...
- You are a supportive coach, NOT a therapist or doctor. Never diagnose, never claim to treat any condition, never give medical advice.
- Never affirm or normalize a harmful intention (self-harm, harming others). If present, route to help — do not reframe it.
...
const MIND_COACH_SYSTEM = `You are the user's mood coach — warm, brief, specific, never clinical. The user is opening the chat inside an app where you can see their last 7 days of mood, anxiety, feelings, triggers, and notes.
...
const MIND_ACTIONS_SYSTEM = `You are the user's mood coach writing a 3-checkin plan. Output 1-3 specific actions, each with:
...
- archetype is one of: breathing_60s, reframe_thought, name_emotion, walk_5min, text_someone, intention_set, log_checkin
...
async function getOrGenAiHeroInsight(deviceId, openai, statsPayload) {
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: MIND_HERO_SYSTEM },
...
async function getOrGenAiReads(deviceId, openai, readsPayload) {
  const hash = contentHash({
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: MIND_AI_READS_SYSTEM },
        { role: 'user',   content: JSON.stringify(readsPayload) },
      ],
    });
    const parsed = JSON.parse(completion.choices?.[0]?.message?.content || '{}');
...
// ════════════════════════════════════════════════════════════════════
async function loadAnalysisV2(deviceId, periodDays, { openai } = {}) {
  ...
  const dailyLogs    = buildDailyLogs(periodCheckins.length ? periodCheckins : allCheckins);
  ...
  if (openai && periodStats.total_checkins >= 1) {
    [heroInsight, aiReads] = await Promise.all([
      getOrGenAiHeroInsight(deviceId, openai, heroPayload),
