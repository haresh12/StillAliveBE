...
async function computeSleepCorrelation(deviceId, mindCheckins) {
  try {
    ...
    console.warn('[mind-analytics] sleep corr failed:', err.message);
    return null;
  }
}
...
async function getOrGenAiHeroInsight(deviceId, openai, statsPayload) {
  ...
  try {
    ...
    console.warn('[mind-analytics] hero insight LLM failed:', err.message);
    return null;
  }
}
...
async function getOrGenAiReads(deviceId, openai, readsPayload) {
  ...
  try {
    ...
    console.warn('[mind-analytics] ai_reads LLM failed:', err.message);
    return [];
  }
}
