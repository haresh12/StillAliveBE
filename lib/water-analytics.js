...
function computeHydrationScore({ logs, target_ml, days = 7 }) {
  ...
  for (const log of logs) {
    const d = log.date || (log.logged_at ? _localDateStr(new Date(getMs(log.logged_at))) : null);
    if (!d) continue;
...
function computeDailyCurve({ logs, target_ml, dateKey }) {
  const todaysLogs = logs.filter(l => {
    const d = l.date || (l.logged_at ? _localDateStr(new Date(getMs(l.logged_at))) : null);
    return d === dateKey;
  });
...
function computeAhaMoments(logs, hydrationScore, target_ml) {
  ...
  const days          = new Set(logs.map(l => l.date || (l.logged_at ? _localDateStr(new Date(getMs(l.logged_at))) : null)).filter(Boolean));
...
async function generateAiReads(logs, target_ml, hydrationScore, openai, deviceId, language = 'en') {
  const breakdown = computeDrinkBreakdown(logs);
  const days      = new Set(logs.map(l => l.date || (l.logged_at ? _localDateStr(new Date(getMs(l.logged_at))) : null)).filter(Boolean));
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: appendLanguageInstruction(`${WATER_AI_READS_SYSTEM_PROMPT}${hkRule}`, language) },
