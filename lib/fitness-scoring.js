...
function sessionDurationMin(w) {
  // Prefer the user-entered duration_min (POST /log captures it directly).
  // Fall back to start/end timestamps from the legacy session model.
  if (Number.isFinite(w?.duration_min) && w.duration_min > 0) {
    return Math.round(w.duration_min);
  }
...
/**
 * Cardio / iso / interval / carry aggregates over a window of workouts.
 * Pulls from the dimensional fields written by POST /log:
 *   cardio_distance_m, cardio_duration_sec,
 *   iso_duration_sec,
 *   interval_rounds, interval_work_sec,
 *   carry_distance_m
 * Workouts that pre-date the entry_type model (or strength-only workouts)
 * have these fields missing — treated as 0, never undefined.
 */
function deriveNonStrengthStats(workouts) {
  let cardio_distance_m = 0, cardio_duration_sec = 0, cardio_sessions = 0;
  let iso_duration_sec = 0, iso_sessions = 0;
  let interval_rounds = 0, interval_work_sec = 0, interval_sessions = 0;
  let carry_distance_m = 0, carry_sessions = 0;
  for (const w of (workouts || [])) {
    if (w?.cardio_distance_m > 0 || w?.cardio_duration_sec > 0) {
      cardio_distance_m  += +w.cardio_distance_m  || 0;
      cardio_duration_sec += +w.cardio_duration_sec || 0;
      cardio_sessions += 1;
    }
    if (w?.iso_duration_sec > 0) {
      iso_duration_sec += +w.iso_duration_sec || 0;
      iso_sessions += 1;
    }
    if (w?.interval_rounds > 0) {
      interval_rounds   += +w.interval_rounds   || 0;
      interval_work_sec += +w.interval_work_sec || 0;
      interval_sessions += 1;
    }
    if (w?.carry_distance_m > 0) {
      carry_distance_m += +w.carry_distance_m || 0;
      carry_sessions += 1;
    }
  }
  return {
    cardio: {
      total_distance_km: Math.round(cardio_distance_m / 100) / 10, // 0.1 km
      total_duration_min: Math.round(cardio_duration_sec / 60),
      sessions: cardio_sessions,
    },
    iso: {
      total_duration_min: Math.round(iso_duration_sec / 60),
      sessions: iso_sessions,
    },
    interval: {
      total_rounds: interval_rounds,
      total_work_min: Math.round(interval_work_sec / 60),
      sessions: interval_sessions,
    },
    carry: {
      total_distance_m: carry_distance_m,
      sessions: carry_sessions,
    },
  };
}

/**
 * Lifetime stats since anchor — the user-facing "I've done X" milestone.
 * Pulls from the FULL workout collection (not the windowed list), so the
 * caller MUST pass `lifetimeWorkouts`, not the range-filtered subset.
 *
 *   total_hours      — sum of duration_min / 60, rounded to 0.1h
 *   total_sessions   — count of workouts
 *   days_active      — distinct calendar dates with a workout
 *   sessions_with_duration — how many workouts had a duration_min logged
 *                            (used by FE to dim the hours number when
 *                            it's mostly inferred from few data points)
 */
function deriveLifetimeStats(lifetimeWorkouts) {
  let totalMin = 0, n = 0, sWithDur = 0;
  ...
  return {
    total_hours: Math.round((totalMin / 60) * 10) / 10,   // 0.1h precision
    total_minutes: totalMin,
    total_sessions: n,
    days_active: dateSet.size,
    sessions_with_duration: sWithDur,
  };
}

/**
 * Session-duration stats from the user-entered `duration_min` on each
 * workout doc. Skips workouts without it so the avg is honest.
 */
function deriveSessionDurationStats(workouts) {
  let total = 0, n = 0;
  for (const w of (workouts || [])) {
    if (Number.isFinite(w?.duration_min) && w.duration_min > 0) {
  ...
  return {
    avg_session_duration_min: n > 0 ? Math.round(total / n) : null,
    total_session_duration_min: total,
...
module.exports = {
  ...
  // New (2026-05-24) — entry_type dimensional aggregates
  deriveNonStrengthStats,
  deriveSessionDurationStats,
  deriveLifetimeStats,
};
