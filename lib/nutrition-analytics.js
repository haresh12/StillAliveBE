...
async function generateNarrative(openai, MODELS, summaryJson, language = 'en') {
  try {
    const completion = await openai.chat.completions.create({
      model: MODELS.fast,
      max_completion_tokens: 350,
      messages: [
        { role: 'system', content: appendLanguageInstruction(ANALYTICS_SYSTEM_PROMPT, language) },
...
async function generateAiReads(openai, MODELS, summary, deviceId, language = 'en') {
  const { withHKEnrichment, HK_PROMPT_RULE } = require('./healthkit/analytics-helper');
  let userMsg = `Input:\n${JSON.stringify(summary)}\n\nReturn the JSON object.`;
  let hkRule = '';
  try {
    const enriched = await withHKEnrichment({ deviceId, coach: 'nutrition', payload: summary });
    userMsg = `Input:\n${enriched}\n\nReturn the JSON object.`;
    hkRule = `\n\n${HK_PROMPT_RULE}`;
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: appendLanguageInstruction(`${AI_READS_SYSTEM_PROMPT}${hkRule}`, language) },
...
async function generateActionPrescription(openai, MODELS, summary, language = 'en') {
  try {
    const completion = await openai.chat.completions.create({
      model: MODELS.fast,
      max_completion_tokens: 1200,
      messages: [
        { role: 'system', content: appendLanguageInstruction(ACTIONS_SYSTEM_PROMPT, language) },
        { role: 'user',   content: `Input:\n${JSON.stringify(summary)}\n\nReturn the JSON object.` },
      ],
      response_format: { type: 'json_object' },
    });
...
async function buildAnalysisPayload(deviceId, range, openai, MODELS, opts = {}) {
  ...
  const cacheKey = `${deviceId}::${range}::${clampStartDate || ''}`;
  ...
  if (!hero_insight && openai && MODELS) {
    ...
    hero_insight = await generateNarrative(openai, MODELS, summary, language);
    if (hero_insight) {
  ...
  if (!ai_reads_llm && openai && MODELS && stats.days_logged >= 1) {
    ...
    ai_reads_llm = await generateAiReads(openai, MODELS, readsSummary, deviceId, language);
    if (ai_reads_llm) {
      readsCache.set({
...
async function answerAsk(openai, MODELS, deviceId, question, language = 'en') {
  const payload = await buildAnalysisPayload(deviceId, '30', openai, MODELS, { language });
  const summary = {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: appendLanguageInstruction(ASK_SYSTEM, language) },
