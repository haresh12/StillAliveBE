...
async function generateAiReads(openai, MODELS, summary, deviceId) {
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: `${AI_READS_SYSTEM_PROMPT}${hkRule}` },
...
async function buildAnalysisPayload(deviceId, range, openai, MODELS, opts = {}) {
  ...
  if (!ai_reads_llm && openai && MODELS && stats.days_logged >= 1) {
    ...
    ai_reads_llm = await generateAiReads(openai, MODELS, readsSummary, deviceId);
    if (ai_reads_llm) {
      readsCache.set({
