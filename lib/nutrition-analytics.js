...
async function buildDailyAggregates(deviceId, days = 90, nutDocRef = null, utcOffsetMinutes = 0) {
  const cutoff = daysAgo(days);
  // nutDocRef lets the big-change nutrition agent reuse this on its OWN namespace
  // (wellness_bc_users/.../agents/nutrition). Defaults to the legacy nutDoc → unchanged behavior.
  const snap = await (nutDocRef || nutDoc(deviceId)).collection('food_logs')
  ...
  for (const d of snap.docs) {
    ...
    // Hour-of-day in the USER's local time (utcOffsetMinutes), not the server's (UTC on Fly) — so
    // peak_hour / evening_kcal_pct ("% after 7pm") reflect when the user actually ate. Default 0 = legacy.
    const hr = new Date((millis(log.logged_at) || Date.now()) + utcOffsetMinutes * 60000).getUTCHours();
  ...
  for (const date of Object.keys(byDate)) {
    ...
    day.quality_avg = day.qualities.length
      ...
      : null;
    // Whole/processed buckets (food_quality_score ≥70 = whole, ≤40 = ultra-processed) captured BEFORE
    // qualities is deleted → powers the range-level whole-vs-processed ratio (a NOVA/UPF proxy).
    day.q_total = day.qualities.length;
    day.q_whole = day.qualities.filter(x => x >= 70).length;
    day.q_processed = day.qualities.filter(x => x <= 40).length;
...
function buildRangeStats(byDate, dates, calTarget, protTarget, todayStr) {
  if (!dates.length) {
    return {
      days_logged: 0, completed_days: 0, avg_kcal: 0, avg_protein: 0, avg_carbs: 0, avg_fat: 0,
  ...
  // "On target" only judges days that are OVER. The in-progress current day is excluded —
  // you can't be "under target" on a day that isn't finished yet (kills the "0 on target" bug).
  const completed = todayStr ? dates.filter(d => d < todayStr) : dates;
  const calHit = completed.filter(d => byDate[d].kcal >= calTarget * 0.9 && byDate[d].kcal <= calTarget * 1.1).length;
  const protHit = completed.filter(d => byDate[d].p >= protTarget * 0.9).length;
...
async function generateNarrative(openai, MODELS, summaryJson) {
  try {
    ...
    log.warn('[analytics] narrative gen failed:', err.message);
    return null;
  }
}
...
async function generateAiReads(openai, MODELS, summary) {
  try {
    ...
    log.warn('[analytics] ai-reads gen failed:', err.message);
    return null;
  }
}
...
async function generateActionPrescription(openai, MODELS, summary) {
  try {
    ...
    log.warn('[analytics] action-rx gen failed:', err.message);
    return null;
  }
}
...
async function buildAnalysisPayload(deviceId, range, openai, MODELS, opts = {}) {
  const clampStartDate = opts.clampStartDate || null;
  const effectiveDays  = opts.effectiveDays || null;
  // cacheNs keeps the big-change ('bc') payload from colliding with the legacy one for the same device.
  const cacheKey = `${opts.cacheNs || ''}${deviceId}::${range}::${clampStartDate || ''}`;
  const cached = lruGet(cacheKey);
  if (cached) return cached;

  const RANGE_MAP = { '7': 7, '30': 30, '90': 90, '365': 365 };
  const rangeDays = effectiveDays || RANGE_MAP[range] || 7;
  const aggDays   = Math.max(rangeDays * 2, 60);  // pull extra for delta vs prior

  // Setup (nutDocRef = bc namespace when called from the big-change nutrition agent; else legacy)
  const _nd = opts.nutDocRef || nutDoc(deviceId);
  const [setupSnap] = await Promise.all([_nd.get()]);
  ...
  // Aggregations — forward nutDocRef so the bc nutrition agent aggregates its OWN namespace
  // (default null → legacy nutDoc, unchanged for legacy callers).
  const byDate = await buildDailyAggregates(deviceId, aggDays, opts.nutDocRef || null, opts.utcOffsetMinutes || 0);
  const allDates = Object.keys(byDate).sort();
  const rawCutoffStr = daysAgo(rangeDays - 1);
  // Clamp window start to the agent's setup date — pre-registration days
  // must never appear on charts.
  const cutoffStr = clampStartDate && clampStartDate > rawCutoffStr ? clampStartDate : rawCutoffStr;
  ...
  const stats = buildRangeStats(byDate, currentDates, calTarget, protTarget, opts.todayStr || null);
  const delta = {
  ...
  try {
    ...
    const userMetrics = {
      ...
      protein_hit_pct: stats.completed_days > 0 ? Math.round((stats.protein_hit_days / stats.completed_days) * 100) : null,
      cal_hit_pct:     stats.completed_days > 0 ? Math.round((stats.cal_hit_days / stats.completed_days) * 100) : null,
    ...
    log.warn('[analytics] cohort lookup failed (non-fatal):', err.message);
  }
  ...
  const cacheRef = _nd.collection('analysis_cache').doc(`${range}_hero`);
  ...
  if (!hero_insight && openai && MODELS) {
    const summary = {
      ...
      protein_hit: `${stats.protein_hit_days}/${stats.completed_days}`,
      cal_hit: `${stats.cal_hit_days}/${stats.completed_days}`,
  ...
  const readsCache = _nd.collection('analysis_cache').doc(`${range}_reads`);
  ...
  // Real macro-balance (0-100): how even the P/C/F split is by calories. Computed ONCE so the grade
  // LETTER (score_grade) and the score NUMBER (nutrition_score) agree — score_grade used to hardcode
  // macro_balance=50, which made the letter disagree with the score it sits next to.
  const _totalMacroCal = stats.avg_protein * 4 + stats.avg_carbs * 4 + stats.avg_fat * 9;
  const macroBalanceScore = _totalMacroCal > 100
    ? Math.round(Math.min((Math.min(stats.avg_protein * 4, stats.avg_carbs * 4, stats.avg_fat * 9) / _totalMacroCal) / 0.2, 1) * 100)
    : 50;

  // ── Nutrition-NATIVE aggregates (zero logging change; computed from already-stored fields) ──
  // meal_breakdown: WHERE the day's calories + protein land (breakfast/lunch/dinner/snack) — and the
  //   per-meal PROTEIN distribution (even spread ≈ 0.4 g/kg or ~30 g/meal beats backloading at dinner).
  // food_quality: whole-vs-processed share from food_quality_score (a NOVA/UPF proxy fitness can't have).
  const _mealAgg = {
    breakfast: { kcal: 0, p: 0, n: 0 },
    lunch:     { kcal: 0, p: 0, n: 0 },
    dinner:    { kcal: 0, p: 0, n: 0 },
    snack:     { kcal: 0, p: 0, n: 0 },
  };
  let _qWhole = 0, _qProcessed = 0, _qTotal = 0, _qScoreSum = 0;
  for (const d of currentDates) {
    const day = byDate[d];
    if (!day) continue;
    for (const it of day.items || []) {
      const mt = _mealAgg[it.meal] ? it.meal : "snack";
      _mealAgg[mt].kcal += it.kcal || 0;
      _mealAgg[mt].p    += it.p || 0;
      _mealAgg[mt].n    += 1;
    }
    _qTotal     += day.q_total || 0;
    _qWhole     += day.q_whole || 0;
    _qProcessed += day.q_processed || 0;
    if (day.quality_avg != null) _qScoreSum += day.quality_avg;
  }
  const _loggedDays = Math.max(stats.days_logged || 0, 1);
  const _totMealKcal = Object.values(_mealAgg).reduce((s, v) => s + v.kcal, 0) || 1;
  const meal_breakdown = ["breakfast", "lunch", "dinner", "snack"].map((meal) => {
    const v = _mealAgg[meal];
    return {
      meal,
      kcal_per_day:    Math.round(v.kcal / _loggedDays),
      protein_per_day: Math.round(v.p / _loggedDays),
      avg_kcal:        v.n ? Math.round(v.kcal / v.n) : 0,
      kcal_pct:        Math.round((v.kcal / _totMealKcal) * 100),
      count:           v.n,
    };
  });
  const food_quality = _qTotal > 0 ? {
    avg_score:     Math.round(_qScoreSum / Math.max(stats.days_logged || 1, 1)),
    whole_pct:     Math.round((_qWhole / _qTotal) * 100),
    processed_pct: Math.round((_qProcessed / _qTotal) * 100),
    total_logs:    _qTotal,
  } : null;
  ...
  const payload = {
    ...
    score_grade: gradeForScore(
      computeNutritionScore({
        // Adherence is over COMPLETED days only — an unfinished current day can't be "off target".
        // Before any day completes there's nothing to score, so pass a neutral 50 (not a penalising 0).
        calorie_adherence: stats.completed_days > 0 ? Math.round((stats.cal_hit_days / stats.completed_days) * 100) : 50,
        protein_adherence: stats.completed_days > 0 ? Math.round((stats.protein_hit_days / stats.completed_days) * 100) : 50,
        streak,
        macro_balance:     macroBalanceScore,
        days_logged:       stats.days_logged,
    ...
    nutrition_score: (() => {
      try {
        // Over COMPLETED days only; neutral 50 before any day finishes (mirrors score_grade above).
        const proteinHitPct = stats.completed_days > 0 ? (stats.protein_hit_days / stats.completed_days) * 100 : 50;
        const calHitPct     = stats.completed_days > 0 ? (stats.cal_hit_days / stats.completed_days) * 100 : 50;
        return computeNutritionScore({
          calorie_adherence: Math.round(calHitPct),
          protein_adherence: Math.round(proteinHitPct),
          streak,
          macro_balance:     macroBalanceScore, // SAME value the grade letter uses (no more divergence)
          days_logged:       stats.days_logged,
          elapsed_days:      effectiveDays,
        });
      } catch { return null; }
