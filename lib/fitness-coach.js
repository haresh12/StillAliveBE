...
function profileLine(ob) {
  ...
  return p.length
    ? `\nWHO THEY ARE (from onboarding — personalize to this, e.g. their goal weight & fitness goals; never re-ask what's here): ${p.join(", ")}.`
    : "";
}
...
module.exports = async function fitnessCoach(req, res) {
  const { deviceId, message, weightUnit } = req.body || {};
  ...
  try {
    ...
    const [histSnap, fSnap, obSnap, yesterday, memories, hkText] = await Promise.all([
    ...
    const messages = [
      { role: "system", content: appendLanguageInstruction(buildSystemPrompt(setup, unit) + profile + memoryBlock + yesterday + hkBlock, language) },
      ...history,
