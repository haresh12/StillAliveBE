...
const { appendLanguageInstruction, resolveUserLanguage } = require('./i18n-prompt');
...
const DEFAULTS = {
  ...
  LOGS_ORDER_FIELD: 'logged_at', // override per agent if different (e.g. fasting uses 'started_at')
};
...
function buildPrompt(deps, slots, recentlyHandled, setup, crossAgentCtx = '') {
  ...
  const candidateSummary = slots.map((c, i) => {
    ...
    return `[${role}] archetype=${c.archetype}, score=${c.score}, category=${c.category}\n` +
           `  data_context: "${c.proof_text}" (current=${c.proof.value}, target=${c.proof.threshold}, source=${c.proof.citation})\n` +
  ...
  return [
    ...
    `     "data_trigger": "<≤80 chars — exact data point that triggered this action, e.g. 'Avg sleep: 5.2h · below 7h target'>",`,
    `     "surprise_hook": "<≤80 chars — one non-obvious insight from the data>",`,
    `     "text": "<≤96 chars — what to do, when, where>",`,
    `     "proof_body": "<≤140 chars — cite exact numbers + research source>",`,
    `     "score_impact": "<≤50 chars — e.g. '+4 to Sleep score if done 3 nights'>",`,
    ...
    "• Every data_trigger MUST be plain English a user reads directly (e.g. 'Avg sleep 5.2h · below 7h target', 'No fasts logged this week'). NEVER use internal variable names, underscores, or 'threshold=' — users see this text on screen.",
    "• Every surprise_hook must be a non-obvious data point.",
    "• score_impact must estimate the point gain realistically (+2 to +8 range). Reference the specific agent score, not 'overall'.",
    ...
    setupStr,
    "",
    ...(crossAgentCtx ? ["━━━ CROSS-AGENT CONTEXT ━━━", crossAgentCtx, "Use this data to make at least one action directly reference a cross-agent pattern (e.g. poor sleep → lighter workout, low hydration → adjust intensity).", ""] : []),
    "CANDIDATES:",
    candidateSummary,
  ].join("\n");
}
...
async function generateActionBatch(deps) {
  ...
  try {
    ...
    // Pull recent logs — field name is agent-specific (fasting uses 'started_at', others use 'logged_at')
    const wSnap = await logsCol(deviceId)
      .orderBy(cfg.LOGS_ORDER_FIELD, "desc")
      .limit(150)
      .get();
    ...
    // Compute candidates (agent-specific). deviceId is passed so wrappers
    // can read cross_agent/today_signals without breaking the sandbox law.
    const ctx = { setup, recentActions, deviceId };
    ...
    const batchKey = `${_localDateStr()}_${Date.now()}`;
    ...
    // ── Cross-agent context (optional enricher per agent) ──
    let crossAgentCtx = '';
    if (typeof crossAgentEnricher === 'function') {
      try { crossAgentCtx = await crossAgentEnricher(deviceId) || ''; } catch {}
    }

    // ── HealthKit context (auto-imported signals for THIS coach only) ──
    // Reads /agents/{agentName}/healthkit_imports — same sandbox law as
    // cross-agent (own-data only here, sibling-data only via crossAgentCtx).
    let hkCtx = '';
    try {
      const { buildHKContext } = require('./healthkit/context-builder');
      hkCtx = await buildHKContext({ db: db(), deviceId, coach: agentName, days: 7 });
    } catch { /* HK enrichment is best-effort */ }
    if (hkCtx) {
      crossAgentCtx = (crossAgentCtx ? `${crossAgentCtx}\n` : '') + `[HEALTHKIT] ${hkCtx}`;
    }

    // ── AI for copy ──
    const prompt = buildPrompt({ agentName, setupSummary }, slots, recentActions, setup, crossAgentCtx);
    // Localize: post-save/cron path → resolve user language from Firestore profile.
    // English = no-op (cache hit preserved). Non-EN appends a language directive.
    let _language = 'en';
    try { _language = await resolveUserLanguage(db(), deviceId); } catch {}
    const localizedPrompt = appendLanguageInstruction(prompt, _language);
    let aiActions = [];
    let weeklyFocus = "";
    try {
    ...
    log.error(`[${agentName}] generateActionBatch:`, e);
    throw e;
  } finally {
...
async function gradeActions(deps) {
  ...
  try {
    ...
    for (const doc of snap.docs) {
      ...
      if (grader) {
        try {
          ...
          log.error(`[${agentName}] grader ${sc.type}:`, e);
        }
      }
    ...
    log.error(`[${agentName}] gradeActions:`, e);
  }
}
...
async function buildResponseShape(deps) {
  ...
  // ── Legacy-shape aliases (back-compat for FE ActionsTabs) ──────────
  // All 6 agent Actions tabs (Mind/Fitness/Sleep/Nutrition/Water/Fasting)
  // were authored against a `{ actions, history, cadence, stats }` shape
  // that predates the v2 engine. Rather than rewriting 6 FE files, the
  // engine now emits BOTH shapes so consumers can pick. v2 readers still
  // see `active/completed/skipped/spotlight/meta/track_record`; legacy
  // readers see `actions/history/cadence/stats` populated below.
  const completed_total_legacy = allRecent.filter(a => a.status === 'completed' || a.status === 'done').length;
  const skipped_total_legacy   = allRecent.filter(a => a.status === 'skipped' || a.status === 'cancelled').length;
  const followThroughDenom = completed_total_legacy + skipped_total_legacy;
  const lastGenIso = inBatch[0]?.generated_at ? toIso(inBatch[0].generated_at) : null;

  const history = allRecent
    .filter(a => a.status === 'completed' || a.status === 'done' || a.status === 'skipped' || a.status === 'cancelled')
    .slice(0, 12)
    .map(a => {
      const cancelled = a.status === 'skipped' || a.status === 'cancelled';
      const tsIso = a.completed_at || a.skipped_at || a.graded_at;
      const tsMs = tsIso ? new Date(tsIso).getTime() : null;
      return {
        id: a.id,
        title: a.title || a.text || 'Action',
        date_label: tsMs ? new Date(tsMs).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—',
        completed_at: tsIso || null,
        outcome: a.outcome_text || (cancelled ? 'Cancelled' : `${a.hit_count ?? a.hit_rate ?? 0}/${a.target_count ?? 1} hit`),
        status: cancelled ? 'cancelled' : 'completed',
      };
    });

  // cadence.status stays falsy when there's no live batch — fitness FE
  // only renders the next-review block when status === 'live', so this
  // keeps fitness intact while populating the mind-shape counters.
  const cadence = {
    status: batchKey ? 'live' : 'pending',
    checkins_so_far:  logCount,
    sessions_so_far:  logCount,
    until_refresh:    sessionsUntilNext,
    days_until_next:  sessionsUntilNext,
    last_gen_checkin: lastGenIso ? logCount - progressToNext : 0,
    next_review_at:   null,
    next_review_label: null,
  };
  ...
  return {
    ...
    stats: {
      ...
      completed_total:     completed_total_legacy,
      skipped_total:       skipped_total_legacy,
      cancelled_total:     skipped_total_legacy,
      total_checkins:      logCount,
      follow_through_pct:  followThroughDenom > 0
        ? Math.round((completed_total_legacy / followThroughDenom) * 100)
        : 0,
    },
  };
}
...
function mountActionRoutes(router, deps) {
  ...
  function queueGeneration(deviceId, opts = {}) {
    ...
    generateActionBatch({
      ...
      .catch((err) => {
        log.error(`[${agentName}] queueGeneration error:`, err);
        _enqueueLocks.delete(k);
  ...
  router.get("/actions", async (req, res) => {
    ...
    try {
      ...
      log.error(`[${agentName}] GET /actions:`, e);
  ...
  router.get("/actions/history", async (req, res) => {
    ...
    try {
      ...
      log.error(`[${agentName}] GET /actions/history:`, e);
      return res.status(500).json({ error: "server error" });
    }
  });

  // POST /action/:id/complete
  // Stores rich outcome data so cross-agent (Insights tab) can later read
  // both ACCEPTED + CANCELLED outcomes across all 6 agents.
  router.post("/action/:id/complete", async (req, res) => {
    ...
    try {
      ...
      await actionsCol(deviceId).doc(id).update({
        ...
        outcome: "accepted",
        outcome_grade: "kept",
        outcome_source: source || "user_tap",
        outcome_recorded_at: now,
      });
      gradeActions({ agentName, deviceId, actionsCol, logsCol, graders, admin, db }).catch(() => {});
      return res.json({ success: true, outcome: "accepted" });
    } catch (e) {
      log.error(`[${agentName}] action complete:`, e);
      return res.status(500).json({ error: "server error" });
    }
  });

  // POST /action/:id/skip
  // "skip" + "cancel" map to the same outcome ("cancelled"). Both write
  // the same rich outcome doc so cross-agent reads stay consistent.
  router.post("/action/:id/skip", async (req, res) => {
    const { id } = req.params;
    const { deviceId, reason, source } = req.body;
    if (!deviceId) return res.status(400).json({ error: "deviceId required" });
    try {
      const now = admin.firestore.FieldValue.serverTimestamp();
      await actionsCol(deviceId).doc(id).update({
      ...
      log.error(`[${agentName}] action skip:`, e);
      return res.status(500).json({ error: "server error" });
    }
  });
  ...
  router.post("/action/:id/snooze", async (req, res) => {
    ...
    try {
      ...
      log.error(`[${agentName}] action snooze:`, e);
      return res.status(500).json({ error: "server error" });
    }
  });
  ...
  router.post("/action/:id/feedback", async (req, res) => {
    ...
    try {
      ...
      log.error(`[${agentName}] action feedback:`, e);
      return res.status(500).json({ error: "server error" });
    }
  });
