...
function cacheKey(candidates, language) {
  // Order-independent hash: sort by id, join. Language is part of the key so
  // a Spanish user doesn't get the English-rewritten cache from yesterday.
  const ids = candidates.map(c => c.id).sort().join('|');
  const raw = `${language || 'en'}::${ids}`;
  return Buffer.from(raw).toString('base64').slice(0, 80).replace(/[^A-Za-z0-9]/g, '_');
}

// MAIN
async function humanizeMessages(deviceId, candidates, language) {
  const lang = normalizeLanguage(language);
  if (!candidates?.length) return [];

  const key = cacheKey(candidates, lang);
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: appendLanguageInstruction(`${SYSTEM_SAFETY_PREFIX}\n\n${SYSTEM_PROMPT}`, lang),
        },
    ...
    log.warn('[assistant-llm]', e.message);
    return candidates.map(c => ({ ...c, text: stripJargon(c.raw_text) }));
  }
