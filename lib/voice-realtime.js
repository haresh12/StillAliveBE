...
// ── Briefing cache — the #1 time-to-first-word lever. Building the briefing (6 domain reads +
// plans + cross-insights + Apple Health + past calls) is the slow, variable part of picking up a
// call. The app PREWARMS it when the Talk tab opens (POST /prewarm), so by the time the user taps
// Call it's already here and mintSession only pays for the OpenAI client_secrets round-trip.
// In-flight coalescing: if a prewarm is still building when the call starts, the call AWAITS that
// same promise instead of building twice.
const BRIEF_TTL_MS = Number(process.env.VOICE_BRIEFING_TTL_MS) || 3 * 60 * 1000;
const CALL_BRIEF_WAIT_MS = Number(process.env.VOICE_BRIEFING_CALL_WAIT_MS) || 1200;
const briefCache = new Map(); // `${deviceId}|${focus}` → { text, at, inflight }

function briefingFor(deviceId, focus, utc_offset_minutes) {
  const key = `${deviceId}|${focus || ''}`;
  const c = briefCache.get(key);
  if (c && c.text != null && Date.now() - c.at < BRIEF_TTL_MS) return Promise.resolve(c.text);
  if (c && c.inflight) return c.inflight;
  const inflight = buildBriefing(deviceId, { focus: focus || null, utc_offset_minutes })
    .then((b) => {
      const text = (b && b.text) || '';
  ...
  briefCache.set(key, { ...(c || {}), inflight });
  return inflight;
}

function briefingForCall(deviceId, focus, utc_offset_minutes) {
  const key = `${deviceId}|${focus || ''}`;
  const cached = briefCache.get(key);
  if (cached && cached.text != null) return Promise.resolve(cached.text);
  const inflight = briefingFor(deviceId, focus, utc_offset_minutes);
  return Promise.race([
    inflight,
    new Promise((resolve) => setTimeout(() => resolve(''), CALL_BRIEF_WAIT_MS)),
  ]);
}

// Fire-and-forget warm-up (called by POST /api/voice/prewarm when the Talk tab opens / topic changes).
function prewarmBriefing(deviceId, focus, utc_offset_minutes) {
  const t0 = Date.now();
  briefingFor(deviceId, focus, utc_offset_minutes).then((text) => {
    console.log(`⏱ [voice-realtime] prewarm done in ${Date.now() - t0}ms — briefing ${text.length} chars (device=${deviceId}${focus ? ` focus=${focus}` : ''})`);
  });
}

function invalidateBriefing(deviceId) {
  const prefix = `${deviceId}|`;
  for (const key of briefCache.keys()) {
    if (key === deviceId || key.startsWith(prefix)) briefCache.delete(key);
  }
}
...
const DOMAINS = ['fitness', 'nutrition', 'mind', 'sleep', 'water', 'fasting', 'breath'];
 ...
 */
async function mintSession(deviceId, { mode = 'user', name = '', topic = '', focus = '', maxCallSec: maxOverride, utc_offset_minutes = null } = {}) {
  if (!OPENAI_API_KEY) throw new Error('OPENAI_API_KEY missing');
  const t0 = Date.now();

  // Everything we need before talking to OpenAI, in PARALLEL (was serial — coach lookup, then
  // briefing, then budget). The briefing comes from the prewarm cache when the Talk tab warmed it.
  const [coach, briefingText, maxCallSec] = await Promise.all([
    // The user's CHOSEN coach (onboarding/Settings) → personality + matched voice + name.
    userDoc(deviceId).get()
      .then((uSnap) => { const u = (uSnap && uSnap.exists ? uSnap.data() : {}) || {}; return getCoach(u.coach_id, u.coach_name); })
      .catch((e) => { console.warn('[voice-realtime] coach lookup failed, using default:', e.message); return getCoach(null, null); }),
    // The MOAT: the briefing (profile + per-domain state + plans + focus deep-dive), anchor-clamped.
    briefingForCall(deviceId, focus, utc_offset_minutes),
    // Call length = the REMAINING monthly minutes. The caller (route) passes the value it already
    // computed for the gate so we don't re-read; otherwise derive it. Returning maxCallSec makes the
    // SERVER the source of truth — a stale client can't extend it.
    (typeof maxOverride === 'number' && maxOverride > 0) ? Promise.resolve(maxOverride) : callLengthFor(deviceId),
  ]);
  ...
  console.log(`🟢 [voice-realtime] → OpenAI client_secrets  coach=${coach.name} voice=${coach.voice} model=${REALTIME_MODEL} device=${deviceId} · prep ${tPrep}ms · briefing ${briefingText.length} chars`);
  ...
  console.log(`✅ [voice-realtime] OpenAI ephemeral key minted (${String(clientSecret).slice(0, 6)}…) — ⏱ prep ${tPrep}ms + client_secrets ${Date.now() - tSecret}ms = ${Date.now() - t0}ms total.`);
...
module.exports = { mintSession, runTool, callLengthFor, prewarmBriefing, invalidateBriefing };
