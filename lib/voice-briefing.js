...
// ═══════════════════════════════════════════════════════════════════════════
const { timeContextBlock } = require('./time-context'); // same time-awareness the chat coach uses
...
const DOMAIN_SET = new Set(['fitness', 'nutrition', 'mind', 'sleep', 'water', 'fasting', 'breath']);
const SELF = `http://127.0.0.1:${process.env.PORT || 5001}`;

// 4s cap (was 1.5s): the briefing is PREWARMED in the background when the Talk tab opens (see
// voice-realtime briefingFor cache), so we can afford to actually wait for plans/cross-insights/
// analysis instead of silently dropping them — that silent drop was what made the coach feel generic.
...
const AGENTS = {
  ...
  breath:    { label: 'breathwork', col: 'breath_sessions',  date: (d) => d.date_str, value: (d) => clamp(Math.round((num(d.seconds) / 60 / 5) * 100) || 40, 20, 100) },
};
...
async function buildBriefing(deviceId, opts = {}) {
  ...
  const profile = {
    ...
    height_cm: Number(o.height_cm) > 0 ? Number(o.height_cm) : null,
    height_unit: o.height_unit || null,
    weight_kg: Number(o.weight_kg) > 0 ? Number(o.weight_kg) : null,
  ...
  const active = profile.focus_domains.length ? profile.focus_domains : ['fitness', 'nutrition', 'sleep', 'water', 'mind', 'fasting', 'breath'];
  const wantFocus = opts.focus && DOMAIN_SET.has(opts.focus) ? opts.focus : null;
  // allSettled (not all): one slow/failing endpoint must NEVER block or break the whole briefing — the
  // coach still picks up promptly with whatever resolved. EVERYTHING runs in parallel here (the
  // uploaded plan / Apple Health / focus deep-dive used to run serially AFTER this — that tail alone
  // added seconds to time-to-first-word).
  const settled = await Promise.allSettled([
    ...
    // Their OWN uploaded workout plan (parsed from the photo they snapped in onboarding).
    AGENT_DOC(deviceId, 'fitness').get().then((s) => (s.exists && s.data().uploaded_plan) || null).catch(() => null),
    // Apple Health signals (recovery, sleep, steps, HRV, RHR, weight, workouts).
    healthSignalsText(deviceId).catch(() => ''),
    // Focused call → that domain's REAL analysis so the coach knows their actual numbers.
    wantFocus ? fetchJson(`${SELF}/api/${wantFocus}/analysis?deviceId=${encodeURIComponent(deviceId)}&range=30`) : Promise.resolve(null),
  ]);
  ...
  const uploadedPlan = val(5, null);
  const hkText = val(6, '');
  const focusAnalysis = val(7, null);

  // Prepend the user's REAL local time so the call coach greets/reasons time-appropriately (no
  // "good morning" at 8 PM, no "the day just started" at bedtime). Same block the chat coach appends.
  // TZ priority: the offset the device sent with THIS call → the user's travel-fresh current_tz_offset
  // → the frozen registration offset (resolveAnchor). This is NEVER empty for an onboarded user, so an
  // AI-INITIATED call (voice-outreach passes no device offset) still knows the time of day instead of
  // letting the LLM guess it.
  const tzOff = Number.isFinite(opts.utc_offset_minutes) ? opts.utc_offset_minutes
    : Number.isFinite(u.current_tz_offset) ? u.current_tz_offset
    : Number.isFinite(u.registration_tz_offset) ? u.registration_tz_offset
    : anchor.utcOffsetMinutes;
  const timeBlock = timeContextBlock(tzOff);
  let text = (timeBlock ? timeBlock.trim() + '\n\n' : '') + composeBriefingText(profile, domains, win, learnings);
  ...
  let focus = null;
  if (wantFocus) {
    const dd = deepDiveText(wantFocus, focusAnalysis);
    if (dd) { text += '\n' + dd; focus = wantFocus; }
  }

  console.log(`⏱ [voice-briefing] built in ${Date.now() - t0}ms — ${text.length} chars (name=${profile.name || '?'}, domains=${domains.length}, plans=${plansResp && plansResp.has_plan ? 'y' : 'n'}, cross=${crossResp ? 'y' : 'n'}, hk=${hkText ? 'y' : 'n'}${focus ? `, focus=${focus}` : ''})`);
...
function composeBriefingText(profile, domains, win, learnings = []) {
  ...
  const bodyBits = [
    ...
    profile.weight_kg && `onboarding current weight: ${profile.weight_kg}kg${profile.weight_unit ? ` (user selected ${fmtWeight(profile.weight_kg, profile.weight_unit)})` : ''}`,
  ].filter(Boolean);
  if (bodyBits.length) {
    L.push(`BODY METRICS — ${bodyBits.join('; ')}. If discussing body weight, treat this onboarding current weight as authoritative unless a newer explicit bodyweight log exists. Do not confuse Apple Health/body-signal weight with onboarding weight; name the source if they differ.`);
  }
...
const fmtWeight = (kg, unit) => {
  if (unit === 'lb') return `${Math.round(Number(kg) * 2.2046)}lb`;
  return `${Number(kg)}kg`;
};
