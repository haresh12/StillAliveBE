...
function mountChatStream(router, deps) {
  ...
  router.post("/chat/stream", async (req, res) => {
    ...
    try {
      ...
      // Build per-agent prompt, then append language directive (cache-friendly), then the user's
      // local-time block last (varies by request, so it never disturbs the cached English prefix).
      const built = await buildPrompt(deviceId, message, { proactive_context });
      let systemPrompt = appendLanguageInstruction(built.systemPrompt, language);
      const timeBlock = timeContextBlock(req.body && req.body.utc_offset_minutes);
