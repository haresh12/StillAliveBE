...
//
// 2026-05-27: collectionGroup queries on `food_logs` require a Firestore
// COLLECTION_GROUP index on `logged_at`. The index ships via
// firestore.indexes.json + `firebase deploy --only firestore:indexes`,
// but if it hasn't propagated yet (or is missing in a dev project), the
// query throws FAILED_PRECONDITION and kills the cron. Defensive
// fallback: iterate wellness_users and query each user's own
// food_logs subcollection — uses only single-collection indexes that
// Firestore auto-creates. Slower but never crashes.
async function rebuildCohortAggregates() {
  const t0 = Date.now();
  const cutoff = admin.firestore.Timestamp.fromMillis(Date.now() - 7 * 86400000);

  // Collect logs grouped by user. Try the fast collectionGroup path first;
  // on FAILED_PRECONDITION (index missing/building) fall back to the
  // per-user iteration. Never lets the missing-index error escape.
  ...
  try {
    ...
    usedFallback = true;
    log.warn('[cohort] collectionGroup index missing — falling back to per-user iteration. Run: firebase deploy --only firestore:indexes');
    // Per-user fallback: iterate wellness_users docs whose
    // agents/nutrition.last_log_date >= 7d ago, query each subcollection.
    // No composite index needed (single-collection queries auto-index).
    try {
      const usersSnap = await db().collection('wellness_users').select().limit(2000).get();
      const cutoffMs = Date.now() - 7 * 86400000;
      const tasks = usersSnap.docs.map(async (u) => {
        const deviceId = u.id;
        try {
          const sub = await db().collection('wellness_users').doc(deviceId)
            ...
            .get();
          if (sub.empty) return;
          const arr = sub.docs.map(d => d.data());
          userLogs.set(deviceId, arr);
        } catch (_) { /* per-user failure is non-fatal */ }
      });
      // Throttle: chunks of 20 concurrent reads
      const chunk = 20;
      for (let i = 0; i < tasks.length; i += chunk) {
        await Promise.all(tasks.slice(i, i + chunk));
      }
    } catch (fbErr) {
      log.error('[cohort] fallback iteration failed:', fbErr?.message);
      return; // bail safely — next tick will retry
    }
  }
  if (usedFallback) {
    log.info(`[cohort] fallback path collected ${userLogs.size} users with recent logs`);
  }
