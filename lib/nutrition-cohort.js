...
// ─── Index-free enumeration of recently-active users ─────────────────
// 🚨 NO FIRESTORE INDEXES, EVER (CLAUDE.md / feedback_no_firestore_indexes). The old crons used
// `collectionGroup('food_logs').where('logged_at', …)` to find recently-active users — but a
// collection-group query with a field filter can ONLY be served by a MANUALLY-created
// COLLECTION_GROUP index (the "create it here" link Firestore prints). That is forbidden.
//
// Instead: read the top-level `wellness_users` collection with NO field filter (a plain collection
// read NEVER needs an index), then check each user's `agents/nutrition.last_log_date` marker (already
// stamped on every food log) and filter the window IN MEMORY. Subcollection doc reads are user-scoped
// and indexless. Returns the deviceIds whose last food log is on/after `cutoffDateStr` (YYYY-MM-DD).
async function listRecentlyActiveDeviceIds(cutoffDateStr) {
  const usersSnap = await db().collection('wellness_users').get();
  const ids = [];
  const CHUNK = 50; // bound concurrent subcollection reads
  for (let i = 0; i < usersSnap.docs.length; i += CHUNK) {
    const batch = usersSnap.docs.slice(i, i + CHUNK);
    const results = await Promise.all(batch.map((u) =>
      u.ref.collection('agents').doc('nutrition').get()
        .then((s) => (s.exists ? { id: u.id, last: s.data().last_log_date } : null))
        .catch(() => null),
    ));
    for (const r of results) {
      if (r && typeof r.last === 'string' && r.last >= cutoffDateStr) ids.push(r.id);
    }
  }
  return ids;
}
...
async function rebuildCohortAggregates() {
  ...
  const cutoffDateStr = dateStr(new Date(cutoffMs));

  // Index-free: find recently-active users (no collectionGroup), then read each user's OWN
  // food_logs subcollection and filter the 7-day window in memory (fetch → filter, per the law).
  const deviceIds = await listRecentlyActiveDeviceIds(cutoffDateStr);
  const userLogs = new Map();
  for (const deviceId of deviceIds) {
    const snap = await userDoc(deviceId).collection('food_logs').get();
    const logs = [];
    for (const doc of snap.docs) {
      const d = doc.data();
      const ms = d.logged_at && typeof d.logged_at.toMillis === 'function'
        ? d.logged_at.toMillis()
        : (d.date_str ? new Date(d.date_str).getTime() : 0);
      if (ms >= cutoffMs) logs.push({ date_str: d.date_str, calories: d.calories, protein: d.protein, food_quality_score: d.food_quality_score });
    }
    if (logs.length) userLogs.set(deviceId, logs);
  }
...
async function getUserCohortPercentiles(deviceId, userMetrics, profile) {
  ...
  if (!dist) {
    try {
      ...
      log.warn('[cohort] read failed:', err.message);
      return null;
    }
  }
