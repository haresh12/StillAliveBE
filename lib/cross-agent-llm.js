...
async function generateBriefing(deviceId, ctx) {
  ...
  const r = await callLLM({ model: AI.CHAT_STREAM, systemPrompt: BRIEFING_SYSTEM, userPrompt, max_completion_tokens: 320 });
...
async function generateJournalEntry(deviceId, ctx, dateOverride) {
  ...
  const r = await callLLM({ model: AI.CHAT_STREAM, systemPrompt: JOURNAL_SYSTEM, userPrompt, max_completion_tokens: 280 });
...
async function generateWeeklyNote(deviceId, ctx) {
  ...
  const r = await callLLM({ model: AI.REASONING_PRO, systemPrompt: WEEKLY_SYSTEM, userPrompt, max_completion_tokens: 380 });
  if (!r.parsed) return null;
  const safe = scanOutput(r.parsed.summary || '');
  if (!safe.safe) r.parsed.summary = safe.text;
...
async function generateSetupSuggestion(deviceId, ctx) {
  ...
  const r = await callLLM({ model: AI.CHAT_STREAM, systemPrompt: SETUP_SUGGESTION_SYSTEM, userPrompt, max_completion_tokens: 200 });
  if (!r.parsed) return null;
...
async function generateSkipInterpretation(deviceId, ctx) {
  ...
  const r = await callLLM({ model: AI.CHAT_STREAM, systemPrompt: SKIP_SYSTEM, userPrompt, max_completion_tokens: 180 });
  if (!r.parsed) return null;
