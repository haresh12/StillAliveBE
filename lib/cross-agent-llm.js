...
// ─── SHARED CALL HELPER ────────────────────────────────────────────
async function callLLM({ model, systemPrompt, userPrompt, json = true, max_completion_tokens = 350 }) {
  try {
    const messages = [
      { role: 'system', content: `${SYSTEM_SAFETY_PREFIX}\n\n${systemPrompt}` },
      { role: 'user',   content: userPrompt },
    ];
    const params = { model, max_completion_tokens, messages };
...
function trimContextForPrompt(ctx, opts = {}) {
  ...
  const out = {
    ...
    today_all_done: ctx.today_all_done,
...
TODAY COMES FIRST. The context includes "today_status" (per agent: logged_today, pending_actions[], done) and "today_all_done".
- If ANY set-up agent is NOT done today (not logged yet, or has pending_actions), your briefing MUST focus on finishing TODAY. Name the single most important thing still pending today and motivate the user to complete it now. Your "action" MUST be that pending item.
- Do NOT tell the user to "come back tomorrow", "rest up for tomorrow", or plan tomorrow while anything is still pending today. Tomorrow talk is only allowed when today_all_done is true.
- Only when today_all_done is true may you acknowledge the day is complete and look ahead to tomorrow.
- Pick ONE agent to focus on (the most impactful pending one) — never nag about every agent at once.

Output strict JSON:
{
  "briefing": "2-3 sentences max, ≤55 words. Cite ONE specific number from the user's data. If something is pending today, end by motivating them to finish it today. Otherwise end with one concrete action OR clarifying question.",
...
async function generateBriefing(deviceId, ctx) {
  ...
  const r = await callLLM({ model: 'gpt-4.1-mini', systemPrompt: BRIEFING_SYSTEM, userPrompt, max_completion_tokens: 320 });
...
async function generateJournalEntry(deviceId, ctx, dateOverride) {
  ...
  const r = await callLLM({ model: 'gpt-4.1-mini', systemPrompt: JOURNAL_SYSTEM, userPrompt, max_completion_tokens: 280 });
...
async function generateWeeklyNote(deviceId, ctx) {
  ...
  const r = await callLLM({ model: 'gpt-4.1', systemPrompt: WEEKLY_SYSTEM, userPrompt, max_completion_tokens: 380 });
  if (!r.parsed) return null;
  const safe = scanOutput(r.parsed.summary || '');
  if (!safe.safe) r.parsed.summary = safe.text;
...
async function generateSetupSuggestion(deviceId, ctx) {
  ...
  const r = await callLLM({ model: 'gpt-4.1-mini', systemPrompt: SETUP_SUGGESTION_SYSTEM, userPrompt, max_completion_tokens: 200 });
  if (!r.parsed) return null;
...
async function generateSkipInterpretation(deviceId, ctx) {
  ...
  const r = await callLLM({ model: 'gpt-4.1-mini', systemPrompt: SKIP_SYSTEM, userPrompt, max_completion_tokens: 180 });
  if (!r.parsed) return null;
