...
function computeMindScore({
  ...
  days_logged,
  streak         = 0,
  elapsed_days,               // registration-anchor: days SINCE signup (so day-1 isn't penalised)
  ...
  // ANCHOR-AWARE: frequency = share of days SINCE SIGNUP you checked in (not days/14). A user who
  // registered today and logged today is 100% consistent — never "1/14". Falls back pre-anchor.
  const _logged = days_logged != null ? days_logged : checkin_dates.length;
  const freqScore   = elapsed_days && elapsed_days > 0
    ? clamp((_logged / elapsed_days) * 12, 0, 12)
    : clamp((checkin_dates.length / 14) * 12, 0, 12);
...
//   days_logged
function computeNutritionScore({ calorie_adherence, protein_adherence, streak, macro_balance, days_logged, elapsed_days }) {
  const d = days_logged || 1;
  ...
  // ANCHOR-AWARE CONSISTENCY (registration-anchor law): "consistency" = the share of days SINCE SIGNUP you
  // actually logged — NOT progress toward an absolute 14-day streak. On day 1, logging today = 1/1 = 100%,
  // never a demoralizing 7%. Falls back to the streak ratio only when elapsed days are unknown.
  const consistencyPct = elapsed_days && elapsed_days > 0
    ? clamp(round((d / elapsed_days) * 100), 0, 100)
    : clamp(round(((streak || 0) / 14) * 100), 0, 100);
...
function computeFastingScore({
  ...
  // ANCHOR-AWARE: frequency = share of days SINCE SIGNUP you logged a fast (not days/14), so a user
  // who registered today and logged today reads full frequency, never "1/14". Falls back pre-anchor.
  const freqScore   = elapsed_days && elapsed_days > 0
    ? clamp((d / elapsed_days) * 10, 0, 10)
    : clamp((d / 14) * 10, 0, 10);
