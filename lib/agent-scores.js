...
function maturityFactor(daysLogged) {
  // Softer curve so day-1 users see meaningful score movement as they log.
...
function computeFitnessScore({ consistency, volume, progression, intensity, recovery, days_logged }) {
  const d = days_logged || 1;
  ...
  if (recovery != null) {
    ...
    raw = clamp(
      (volume      || 0) * 0.375 +
      (intensity   || 0) * 0.3125 +
      (consistency || 0) * 0.3125,
      0, 100
    );
  }

  let score = clamp(round(raw * maturityFactor(d)), 0, 100);
  ...
  return {
    ...
    components: {
      ...
      recovery:     recovery != null ? round(recovery) : null,
    },
    days_logged: d,
    // Tells downstream consumers (Home tab, Insights) whether the recovery
    // cap was applied — useful for "your sleep capped this score" messaging.
    recovery_capped: recovery != null && recovery < 50,
  };
}
