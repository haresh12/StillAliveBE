...
// ⚠️ DEPRECATED — kept ONLY for backward callers that haven't migrated
// to the canonical slow curve in lib/{agent}-scoring.js::maturityRamp.
// Per SCORING_CONTRACT_V3.md §2, all 6 agents now use the same slow ramp:
//   0.40 / 0.45 / 0.55 / 0.70 / 0.85 / 0.94 / 1.00 at day 0/1/4/7/14/30/60+
//
// This steeper curve let Day-7 perfect scores reach ~80 ("Thriving") which
// contradicts the foundational law at the top of this file:
//   "Day 7 = ~55"
//
// All 6 per-agent scorers in this file now import their lib's maturityRamp.
// DO NOT add new callers to this function. Remove once parity test confirms
// no in-tree caller remains.
function maturityFactor(daysLogged) {
  if (!daysLogged || daysLogged < 1) return 0.40;
  if (daysLogged < 4)  return 0.65;
  if (daysLogged < 7)  return 0.80;
  if (daysLogged < 14) return 0.90;
  if (daysLogged < 30) return 0.96;
  return 1.00;
}
...
function computeSleepScore({
  ...
  sleep_debt,
  target_hours,
  bedtime_target_hour,             // V3: new — chronotype anchor
  bedtime_variance_std_hours,      // V3: new — last 14d bedtime std
  days_logged,
  days_since_anchor,
  hkSignals = null,                // V3: optional HK enhancement
  use_v2 = true,                   // V3: default ON; pass false for V1 parity testing
}) {
  // V3 path: delegate to lib/sleep-scoring.computeBlendedSleepScore (7 contributors).
  // V1 keys preserved in `components_v1` so legacy FE keeps working.
  if (use_v2) {
    const { computeBlendedSleepScore } = require('./sleep-scoring');
    const r = computeBlendedSleepScore({
      avg_efficiency, avg_duration, avg_quality, avg_energy, avg_latency,
      consistency_score, sleep_debt, target_hours,
      bedtime_target_hour, bedtime_variance_std_hours,
      days_logged, days_since_anchor, hkSignals,
    });
    if (!r) return null;
    return {
      score: r.score,
      label: labelFor(r.score, [[85,'Excellent'],[70,'Good'],[50,'Fair'],[0,'Needs work']]),
      // Expose V1 component keys (Home + Analysis tabs read these)
      components: r.components_v1,
      // V3 enrichments — FE can opt-in via the new explainer sheet
      components_v3: r.components,
      hk_used: r.hk_used,
  ...
  // V1 fallback (kept for migration parity tests + emergency rollback)
  ...
  // Use the SLOWER honest curve (2026-05-24, aligned with fitness + mind):
  // 0.40 / 0.45 / 0.55 / 0.70 / 0.85 / 0.94 / 1.00 at day 0/1/4/7/14/30/60+.
  // The shared maturityFactor here was too steep (0.65 by day 4, 0.80 by
  // day 7) — it let a perfect 7-night logger hit ~80 ("Excellent"), which
  // breaks the foundational rule at the top of this file: "Day 7 = ~55."
  // Sleep now matches the fitness curve via the dedicated lib/sleep-scoring.
  const { maturityRamp: _sleepMaturity } = require('./sleep-scoring');
  const score        = clamp(round(rawScore * _sleepMaturity(d)), 0, 100);
...
function computeMindScore({
  ...
  checkin_dates  = [],
  days_logged,
  days_since_anchor,   // V3: prefer calendar-time over log-count for maturity
  streak         = 0,
  recent_sleep_hours = null,  // cross-agent: avg last 3 nights (null = no data)
  hkSignals      = null,      // V3 §4: optional HK enhancement (HRV proxies anxiety)
}) {
  // V3 contract §2: maturity is CALENDAR-keyed.
  const d = Number.isFinite(days_since_anchor)
    ? days_since_anchor
    : (days_logged || checkin_dates.length || 1);
  let n = mood_scores.length;

  // V3 §4: HK fusion — when user has zero/few manual checkins, HRV provides
  // an anxiety proxy. Healthy adult HRV (SDNN) baseline: 30-50 ms. Higher =
  // better parasympathetic tone = lower anxiety. Inject a synthetic anxiety
  // reading so the scorer has something to work with on days without checkins.
  let hkApplied = false;
  let effectiveAnxiety = anxiety_scores;
  if (hkSignals && Number.isFinite(hkSignals.hrv_overnight_ms) && (n < 7 || anxiety_scores.length < 7)) {
    // Map HRV → synthetic anxiety on 1-5 scale (5 = high anxiety = low HRV)
    const hrv = hkSignals.hrv_overnight_ms;
    let synthAnx;
    if (hrv >= 60)      synthAnx = 1.5;
    else if (hrv >= 45) synthAnx = 2.0;
    else if (hrv >= 35) synthAnx = 2.5;
    else if (hrv >= 25) synthAnx = 3.0;
    else                synthAnx = 3.5;
    // Append synthetic reading (only fills the gap; doesn't override manual)
    effectiveAnxiety = [...anxiety_scores, synthAnx];
    if (n === 0 && mood_scores.length === 0) {
      // Use a neutral mood reading so the function returns a score at all
      // (otherwise it returns null on n===0). Mood of 3 = neutral.
      n = 1;
    }
    hkApplied = true;
  }
  ...
  // V3 §4: when HK provides HRV, `effectiveAnxiety` includes the synthesized
  // reading so this gate has signal even on days the user skipped checkin.
  const recentAnx = effectiveAnxiety.slice(0, Math.min(7, Math.max(1, n)));
  ...
  // ── COMPOSITE ────────────────────────────────────────────────
  // Use the SLOWER honest curve (2026-05-23, aligned with fitness):
  // 0.40 / 0.45 / 0.55 / 0.70 / 0.85 / 0.94 / 1.00 at day 0/1/4/7/14/30/60+.
  // The shared maturityFactor here is too steep (0.65 by day 4) and lets
  // new users hit "Thriving" scores from 4 checkins — contradicts the
  // "score must start at 30-40 and grow" law.
  const { maturityRamp: _mindMaturity } = require('./mind-scoring');
  const raw   = clamp(moodScore + anxScore + trajectoryScore + behaviorScore + sleepImpact, 0, 100);
  const score = clamp(round(raw * _mindMaturity(d)), 0, 100);
...
// ─── FITNESS — Scoring V2 (4-gate, 2026-05-21) ───────────────────────────────
// Data needed:
//   consistency (0-100), volume (0-100), intensity (0-100), days_logged
//   progression (0-100) — kept for analytics, not weighted
//   recovery (0-100|null) — composite sleep×0.6 + (100-anxiety)×0.4 from cross_agent today_signals
//
// V2 formula:
//   Volume 30% · Intensity 25% · Consistency 25% · Recovery 20%
//   When recovery is null (cross-agent signals not available yet), redistribute
//   the 20% pro-rata across the other three (volume 37.5% / intensity 31.25% /
//   consistency 31.25%) so day-1 users still get a real score.
//
// Recovery cap: if recovery is present AND < 50, max score = 65. Protects the
// user from overtraining-driven false-positive "Strong" scores when their
// sleep/HRV says they're under-recovered. Per Israetel + Helms 2018.
function computeFitnessScore({ consistency, volume, progression, intensity, recovery, days_logged, days_since_anchor, hkSignals = null }) {
  // V3 contract §2: maturity is CALENDAR-keyed.
  const d = Number.isFinite(days_since_anchor) ? days_since_anchor : (days_logged || 1);
  if (consistency == null && volume == null) return null;

  // V3 §4: HK fusion — when present, enhance Volume + Consistency components.
  // Volume boost: HK steps fold into the volume signal (caps boost at +10%).
  // Consistency boost: HK workout presence on days without manual logs.
  // Recovery: HK resting-HR baseline drop → boosts recovery component.
  let volumeEnhanced      = volume;
  let consistencyEnhanced = consistency;
  let recoveryEnhanced    = recovery;
  let hkApplied = false;
  if (hkSignals) {
    if (Number.isFinite(hkSignals.steps_last_7d_avg) && hkSignals.steps_last_7d_avg > 5000) {
      // 7d-smoothed step count → volume boost. 10k/day daily floor for full +10%.
      const stepBoost = Math.min(10, ((hkSignals.steps_last_7d_avg - 5000) / 5000) * 10);
      volumeEnhanced = Math.min(100, (volume || 0) + stepBoost);
      hkApplied = true;
    }
    if (Array.isArray(hkSignals.workouts_last_7d) && hkSignals.workouts_last_7d.length > 0) {
      // HK workouts present → consistency baseline lift.
      const hkWorkoutBoost = Math.min(15, hkSignals.workouts_last_7d.length * 3);
      consistencyEnhanced = Math.min(100, (consistency || 0) + hkWorkoutBoost);
      hkApplied = true;
    }
    if (recovery == null && Number.isFinite(hkSignals.hrv_baseline_ms)) {
      // No cross-agent recovery yet — derive a fallback from HRV (40-80ms band → 50-100).
      recoveryEnhanced = Math.max(0, Math.min(100, ((hkSignals.hrv_baseline_ms - 30) / 50) * 100));
      hkApplied = true;
    }
  }

  let raw;
  if (recoveryEnhanced != null) {
    raw = clamp(
      (volumeEnhanced      || 0) * 0.30 +
      (intensity           || 0) * 0.25 +
      (consistencyEnhanced || 0) * 0.25 +
      (recoveryEnhanced    || 0) * 0.20,
      0, 100
    );
  } else {
    // No cross-agent signals yet — redistribute the 20% Recovery slot.
    raw = clamp(
      (volumeEnhanced      || 0) * 0.375 +
      (intensity           || 0) * 0.3125 +
      (consistencyEnhanced || 0) * 0.3125,
      0, 100
    );
  }

  // V3: use the slow canonical ramp from lib/fitness-scoring.js (matches
  // sleep / mind / water / fasting). The shared maturityFactor above was
  // too steep (0.65 at day 4, 0.80 at day 7) and let new users hit "Strong"
  // scores after a week — contradicts the foundational rule at the top of
  // this file: "Day 1 perfect = ~25, Day 7 = ~55, Day 30 = ~90."
  const { maturityRamp: _fitnessMaturity } = require('./fitness-scoring');
  let score = clamp(round(raw * _fitnessMaturity(d)), 0, 100);

  // Under-recovered cap — never let the score say "Strong" when the user's
  // recovery markers are red. Stops false confidence + over-training.
  if (recoveryEnhanced != null && recoveryEnhanced < 50) {
  ...
  return {
    ...
    components: {
      consistency:  round(consistencyEnhanced || consistency || 0),
      volume:       round(volumeEnhanced      || volume      || 0),
      progression:  round(progression  || 0), // retained for analytics; not weighted in V2
      intensity:    round(intensity    || 0),
      recovery:     recoveryEnhanced != null ? round(recoveryEnhanced) : (recovery != null ? round(recovery) : null),
    },
    days_logged: d,
    hk_used: hkApplied,
    recovery_capped: recoveryEnhanced != null && recoveryEnhanced < 50,
  };
}

// ─── NUTRITION ────────────────────────────────────────────────────────────────
// V3: math moved to lib/nutrition-scoring.js so every agent has a dedicated
// lib (per SCORING_CONTRACT_V3.md §1) AND the slow maturity curve is shared
// with the other 5 agents. The legacy 4-input shape used by
// adapters/nutrition.adapter.js is preserved via computeBlendedNutritionScoreLegacy.
function computeNutritionScore({ calorie_adherence, protein_adherence, streak, macro_balance, days_logged, days_since_anchor }) {
  const { computeBlendedNutritionScoreLegacy } = require('./nutrition-scoring');
  // V3 contract §2: maturity is CALENDAR-keyed; legacy fn takes days_logged as
  // its maturity input, so we forward days_since_anchor when present.
  const dForMaturity = Number.isFinite(days_since_anchor) ? days_since_anchor : days_logged;
  const r = computeBlendedNutritionScoreLegacy({
    calorie_adherence, protein_adherence, streak, macro_balance,
    days_logged: dForMaturity,
  });
  if (!r) return null;
  return {
    score: r.score,
    label: labelFor(r.score, [[85,'Excellent'],[70,'Strong'],[55,'Good'],[35,'Building'],[0,'Starting']]),
...
function computeWaterScore({ hydration_adequacy, consistency, chronobiology, beverage_quality, avg_7d_ml, days_logged, days_since_anchor, hkSignals = null }) {
  // V3 contract §2: maturity is CALENDAR-keyed.
  const d = Number.isFinite(days_since_anchor) ? days_since_anchor : (days_logged || 1);
  if (hydration_adequacy == null) return null;

  // V3 §4: HK fusion — active-cal expenditure raises hydration need (Sawka 2007).
  // Each 100 active-kcal ≈ +30ml extra water need. We re-scale hydration_adequacy
  // downward when HK shows high activity but goal wasn't adjusted upstream.
  // Skin temperature elevation is a hot-environment marker → further reduce.
  let hydrationEnhanced = hydration_adequacy;
  let hkApplied = false;
  if (hkSignals) {
    let goalMultiplier = 1.0;
    if (Number.isFinite(hkSignals.active_kcal_today) && hkSignals.active_kcal_today > 200) {
      // 500 kcal → +15% goal need; 1000 kcal → +30%; cap at +50%
      goalMultiplier += Math.min(0.5, (hkSignals.active_kcal_today - 200) / 200 * 0.075);
      hkApplied = true;
    }
    if (Number.isFinite(hkSignals.skin_temp_c) && hkSignals.skin_temp_c > 35.5) {
      // Elevated skin temp → hot environment / fever → +10% need
      goalMultiplier += 0.10;
      hkApplied = true;
    }
    if (goalMultiplier > 1.01) {
      hydrationEnhanced = Math.max(0, Math.min(100, (hydration_adequacy || 0) / goalMultiplier));
    }
  }

  const raw = clamp(
    (hydrationEnhanced  || 0) * 0.35 +
    (consistency        || 0) * 0.25 +
  ...
  // V3: use slow canonical ramp from lib/water-scoring.js. The previous
  // d<3 special-case (0.28) pre-dated the unified maturity contract and
  // double-penalised new users (already covered by maturityRamp(d<4)=0.45).
  const { maturityRamp: _waterMaturity } = require('./water-scoring');
  const score = clamp(round(raw * _waterMaturity(d)), 0, 100);
  ...
  return {
    ...
    components: {
      hydration_adequacy: round(hydrationEnhanced || hydration_adequacy || 0),
    ...
    hk_used: hkApplied,
    citations: {
      hydration_adequacy: 'EFSA 2010, Gandy 2015, Sawka 2007 (HK active-cal adjust)',
...
function computeFastingScore({
  ...
  const raw = clamp(adherenceScore + depthScore + metabolicScore + consistencyScore, 0, 100);

  // V3: use slow canonical ramp from lib/fasting-scoring.js — same curve as
  // fitness/sleep/mind/water/nutrition. The shared maturityFactor was too
  // steep; combined with the d<3 raw cap (28) it pushed Day-1 perfect to
  // ~18, BELOW the contract's 30-40 floor. Fix: apply cap to FINAL score,
  // not raw — maturity ramp already enforces honesty for new users.
  const { maturityRamp: _fastingMaturity } = require('./fasting-scoring');
  let score = clamp(round(raw * _fastingMaturity(d)), 0, 100);

  // Low-completion safety cap: never let the score say "Strong" when the
  // user is breaking >80% of fasts. Applied to FINAL score so it doesn't
  // compound with maturity penalty.
  if (rate7 < 0.20) score = Math.min(score, 35);
